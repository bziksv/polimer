...
Последнее обновление: 2026-09-25 (экспорт Яндекс: фильтр по складам + мин. суммарный остаток в setup/run)
...
| Экспорт Яндекс: фильтр по складам | `bitrix/php_interface/include/catalog_export/yandex_{setup,run}.php` + `polimer_yandex_export_stock.php` |

### Экспорт Яндекс — склады и мин. остаток (2026-09-25)

В настройках любого профиля типа **Yandex** (`Магазин → Настройки → Экспорт данных`) есть блок **«Фильтр по складам (Полимер)»**:

- отметить один или несколько складов;
- задать **минимальное суммарное количество** (по умолчанию 1).

В фид попадают только товары/ТП, у которых сумма `AMOUNT` по выбранным складам ≥ порога. Если склады не отмечены — поведение как раньше (все доступные по галке «только в наличии»).
...
## 12. Каталог и инфоблоки 1С

> **2026-09-16:** зафиксировать текущий [dev.polimer-vrn.ru](https://dev.polimer-vrn.ru/) — сейчас выгружается новая 1С, каталоги с инфоблока могут быть другими, чем на prod.

Сайт **всегда** читает каталог из **IBLOCK 21** (`define("IBLOCK_CATALOG", 21)` в `init.php`; то же в `catalog/index.php` и меню V6). Тип `1c_catalog`, бренды — IBLOCK 28.

1С при обмене может завести **новый** инфоблок «Основной каталог товаров» (уже есть мёртвые 39 и 40 с другими XML_ID). Код сам на него не переключится.

**Снимок 2026-09-16 ~14:00 MSK** (HTML + деревья разделов): `архивы/catalog-snapshot-2026-09-16/`.

На тот момент remote-dev и prod уже разошлись:

| | Dev (`dev`) | Prod (`polimer_vrn_r_db`) |
|---|---|---|
| IBLOCK 21 имя | Товары 1С | Основной каталог товаров |
| XML_ID | `1c_catalog-f87e6b5e-f319-432e-9552-47ff4bc70cfc` | тот же |
| Активных разделов / товаров | 1 / 1 | 727 / 11832 |
| SKU | 22 (старый) **и 41 (новый)** | только 22 |
| Корни 21 | 1153/1492/1496 **выкл.**; живой корень **3876** (конвекторы) | 1153/1492/1496 **вкл.**; 3876 — подраздел сантехники |

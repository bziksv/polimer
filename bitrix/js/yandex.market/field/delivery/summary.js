		...
		getDisplayValueText: function(displayValue) {
			...
			if (displayValue['ORDER_BEFORE'].length > 0) {
				result += ' ' + this.getLang('ORDER_BEFORE_LANG', {
					'ORDER_BEFORE': Utils.escape(displayValue['ORDER_BEFORE']),
			...
			if (hasPeriodFrom || hasPeriodTo) {
				...
				if (hasPeriodFrom && hasPeriodTo) {
					period = Utils.escape(displayValue['PERIOD_FROM']) + '-' + Utils.escape(displayValue['PERIOD_TO']);
					periodForSklon = Utils.escape(displayValue['PERIOD_TO']);
				} else if (hasPeriodFrom) {
					period = Utils.escape(displayValue['PERIOD_FROM']);
					periodForSklon = Utils.escape(displayValue['PERIOD_FROM']);
				} else {
					period = Utils.escape(displayValue['PERIOD_TO']);
					periodForSklon = Utils.escape(displayValue['PERIOD_TO']);
				}
			...
			if (hasPrice) {
				result += ' ' + this.getLang('PRICE_LANG', {
					'PRICE': Utils.escape(displayValue['PRICE']),

...
IncludeModuleLangFile($_SERVER['DOCUMENT_ROOT'].'/bitrix/modules/catalog/load/yandex_setup.php');

require_once $_SERVER['DOCUMENT_ROOT'].'/bitrix/php_interface/include/polimer_yandex_export_stock.php';
...
if (($ACTION == 'EXPORT_EDIT' || $ACTION == 'EXPORT_COPY') && $STEP == 1)
{
	...
	if (isset($arOldSetupVars['STORE_IDS']))
		$STORE_IDS = $arOldSetupVars['STORE_IDS'];
	if (isset($arOldSetupVars['MIN_STORE_QUANTITY']))
		$MIN_STORE_QUANTITY = $arOldSetupVars['MIN_STORE_QUANTITY'];
}
...
if ($STEP > 1)
{
	...
	$STORE_IDS = polimerYandexNormalizeStoreIds(isset($_POST['STORE_IDS']) ? $_POST['STORE_IDS'] : (isset($STORE_IDS) ? $STORE_IDS : []));
	$_REQUEST['STORE_IDS'] = $STORE_IDS;
	if (isset($_POST['MIN_STORE_QUANTITY']) && is_string($_POST['MIN_STORE_QUANTITY']))
		$MIN_STORE_QUANTITY = $_POST['MIN_STORE_QUANTITY'];
	$MIN_STORE_QUANTITY = (!isset($MIN_STORE_QUANTITY) ? 1 : (int)$MIN_STORE_QUANTITY);
	if ($MIN_STORE_QUANTITY < 0)
		$MIN_STORE_QUANTITY = 0;
...
<tr class="heading">
	<td colspan="2">Фильтр по складам (Полимер)</td>
</tr>
<tr>
	<td width="40%" valign="top">Склады для подсчёта остатка</td>
	<td width="60%">
		<?php
		$selectedStoreIds = polimerYandexNormalizeStoreIds(isset($STORE_IDS) ? $STORE_IDS : []);
		$selectedMap = array_fill_keys($selectedStoreIds, true);
		$activeStores = polimerYandexGetActiveStores();
		if (empty($activeStores))
		{
			echo '<span style="color:#a00;">Активных складов не найдено</span>';
		}
		else
		{
			echo '<div style="max-height:220px;overflow:auto;border:1px solid #c6cdd3;padding:8px;background:#fff;">';
			foreach ($activeStores as $store)
			{
				$sid = (int)$store['ID'];
				$checked = isset($selectedMap[$sid]) ? ' checked' : '';
				echo '<label style="display:block;margin:0 0 4px;">'
					.'<input type="checkbox" name="STORE_IDS[]" value="'.$sid.'"'.$checked.'> '
					.htmlspecialcharsbx($store['TITLE']).' <span style="color:#888;">(ID='.$sid.')</span>'
					.'</label>';
			}
			echo '</div>';
			echo '<small style="display:block;margin-top:6px;color:#666;">Если ничего не выбрано — фильтр по складам не применяется (как раньше). '
				.'Если выбраны — в фид попадают только товары, у которых <b>сумма остатков</b> по отмеченным складам '
				.'не меньше минимального количества.</small>';
		}
		?>
	</td>
</tr>
<tr>
	<td width="40%">Минимальное суммарное количество</td>
	<td width="60%">
		<?php
		$minStoreQuantityValue = isset($MIN_STORE_QUANTITY) ? (int)$MIN_STORE_QUANTITY : 1;
		if ($minStoreQuantityValue < 0)
			$minStoreQuantityValue = 0;
		?>
		<input type="text" name="MIN_STORE_QUANTITY" size="5" value="<?=$minStoreQuantityValue;?>">
		<small style="display:block;margin-top:4px;color:#666;">Товар попадает в фид, если сумма остатков по выбранным складам ≥ этого числа (например, 1).</small>
	</td>
</tr>
<tr>
...
if (2 > $STEP)
{
	...
	<input type="hidden" name="SETUP_FIELDS_LIST" value="V,IBLOCK_ID,SITE_ID,SETUP_SERVER_NAME,COMPANY_NAME,SETUP_FILE_NAME,XML_DATA,USE_HTTPS,FILTER_AVAILABLE,DISABLE_REFERERS,EXPORT_CHARSET,MAX_EXECUTION_TIME,CHECK_PERMISSIONS,STORE_IDS,MIN_STORE_QUANTITY">

...
IncludeModuleLangFile($_SERVER['DOCUMENT_ROOT'].'/bitrix/modules/catalog/load/yandex_run.php');
...
$SETUP_VARS_LIST = 'IBLOCK_ID,SITE_ID,V,XML_DATA,SETUP_SERVER_NAME,COMPANY_NAME,SETUP_FILE_NAME,USE_HTTPS,FILTER_AVAILABLE,DISABLE_REFERERS,EXPORT_CHARSET,MAX_EXECUTION_TIME,CHECK_PERMISSIONS,STORE_IDS,MIN_STORE_QUANTITY';
...
$polimerStoreIds = polimerYandexNormalizeStoreIds(isset($STORE_IDS) ? $STORE_IDS : []);
$polimerMinStoreQuantity = isset($MIN_STORE_QUANTITY) ? (float)$MIN_STORE_QUANTITY : 0.0;
if ($polimerMinStoreQuantity < 0)
	$polimerMinStoreQuantity = 0.0;
$polimerStoreFilterActive = !empty($polimerStoreIds);
$polimerStoreAmounts = [];
...
if (empty($arRunErrors))
{
	...
	do
	{
		...
		if (!empty($items))
		{
			if ($polimerStoreFilterActive)
			{
				$idsForStock = [];
				foreach ($items as $stockItem)
				{
					if (!empty($stockItem['OFFERS']) && is_array($stockItem['OFFERS']))
					{
						foreach ($stockItem['OFFERS'] as $stockOffer)
						{
							$oid = (int)($stockOffer['ID'] ?? 0);
							if ($oid > 0)
								$idsForStock[$oid] = $oid;
						}
					}
					...
					{
						$pid = (int)($stockItem['ID'] ?? 0);
						if ($pid > 0)
							$idsForStock[$pid] = $pid;
					}
				}
				$polimerStoreAmounts = polimerYandexGetStoreAmountSums(array_values($idsForStock), $polimerStoreIds);
				unset($idsForStock, $stockItem, $stockOffer, $oid, $pid);
			}
			...
			foreach ($itemIdsList as $id)
			{
				...
				if ($row['TYPE'] == Catalog\ProductTable::TYPE_SKU && !empty($row['OFFERS']))
				{
					...
					{
						if ($polimerStoreFilterActive
							&& !polimerYandexPassStoreFilter((int)$offer['ID'], $polimerStoreAmounts, $polimerMinStoreQuantity)
						)
						{
							continue;
						}
				...
				elseif (isset($simpleIdsList[$id]) && !empty($row['PRICES']))
				{
					...
					if ($polimerStoreFilterActive
						&& !polimerYandexPassStoreFilter((int)$row['ID'], $polimerStoreAmounts, $polimerMinStoreQuantity)
					)
					{
						continue;
					}

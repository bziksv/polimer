...
****************************************************************************/
        ...
        if (isset($arXmlData['OLD_PRICE']))
			$OLD_PRICE = (int)$arXmlData['OLD_PRICE'];
...
?>
    ...
    <tr>
        <td><?=GetMessage('YANDEX_OLD_PRICE_TYPE');?>:</td>
        <td><select class="select_old_price_type" name="OLD_PRICE">
            <option value=""<? echo ($OLD_PRICE == "" || $OLD_PRICE == 0 ? ' selected' : '');?>><?=GetMessage('YANDEX_OLD_PRICE_TYPE_DEFAULT');?></option>
            <option class="wbs24_price_before_discount" value="-2"<?=(-2 == $OLD_PRICE ? ' selected' : '')?>><?=GetMessage('WBS24_SBERMMEXPORT_PRICE_BEFORE_DISCOUNT');?></option>
<?
	if (!empty($arGroups))
	{
		$priceQueryResult = CCatalogGroup::GetListEx(
			array('SORT' => 'ASC'),
			array('ID' => $arGroups),
			false,
			false,
		...
		while ($arRes = $priceQueryResult->Fetch())
		{
			?>
...
?>
     <option value="-1"<?=(-1 == $OLD_PRICE ? ' selected' : '')?>><?=GetMessage('WBS24_SBERMMEXPORT_PRICE_FROM_PROPERTY');?></option>
    </tr>

    <tr>
        <td><?=GetMessage('WBS24_SBERMMEXPORT_PRODUCT_OLD_PRICE_PROPERTIES');?>: </td>
        <td>
            <?=$sbermmSettings->getSelect(
                "COMMON_FIELDS[PRODUCT_OLD_PRICE_PROPERTY]",
                $arIBlock['PROPERTY'],
                $commonFields['PRODUCT_OLD_PRICE_PROPERTY'],
                [
                    'disabled' => ($OLD_PRICE != -1),
                    'default' => GetMessage("YANDEX_SKIP_PROP"),
                ]
            );?>
        </td>
    ...
    <tr>
        <td><?=GetMessage('WBS24_SBERMMEXPORT_OFFER_OLD_PRICE_PROPERTIES');?>: </td>
        <td>
            <?=$sbermmSettings->getSelect(
                "COMMON_FIELDS[OFFER_OLD_PRICE_PROPERTY]",
                $arIBlock['OFFERS_PROPERTY'],
                $commonFields['OFFER_OLD_PRICE_PROPERTY'],
                [
                    'disabled' => ($OLD_PRICE != -1),
                    'default' => GetMessage("YANDEX_SKIP_PROP"),
                ]
    ...
    if (typeof selectPriceTypeHandler === 'undefined') {
        let selectPriceTypeHandler = function() {
            ...
            let oldPriceSelect = document.querySelector('.select_old_price_type');
            let productOldPricePropSelect = document.querySelector('.select_common_fields_product_old_price_property');
            let offerOldPricePropSelect = document.querySelector('.select_common_fields_offer_old_price_property');
            let priceBeforeDiscountOption = document.querySelector('.wbs24_price_before_discount');

            if (priceSelect && productPropSelect && offerPropSelect && priceBeforeDiscountOption) {
                priceSelect.addEventListener('change', () => {
                    ...
                    let priceBeforeDiscount = (priceSelect.value == '');
                    priceBeforeDiscountOption.disabled = !priceBeforeDiscount;
                });
            }

            if (oldPriceSelect && productOldPricePropSelect && offerOldPricePropSelect) {
                oldPriceSelect.addEventListener('change', () => {
                    let priceFromProps = (oldPriceSelect.value == -1);
                    productOldPricePropSelect.disabled = !priceFromProps;
                    offerOldPricePropSelect.disabled = !priceFromProps;
                });
            }

            const Adminhandlers = new Wbs24SbermmexportAdminHandlers();
            Adminhandlers.addCustomOldPriceHandler();
        };

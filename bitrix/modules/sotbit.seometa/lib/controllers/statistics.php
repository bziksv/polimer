...
class Statistics extends Controller
{
                ...
                if ($arCondition && $arrSeometaData = unserialize($arCondition['META'])) {
                    ...
                    if ($section) {
                        ...
                        if ($resSeometaData['META_TITLE']) {
                            $META_TITLE = self::getMeta($resSeometaData['META_TITLE'], $section, $arCondition['INFOBLOCK'], $eventRegions, $sku, $morphyObject);
                        }
                        if ($resSeometaData['META_KEYWORDS']) {
                            $META_KEYWORDS = self::getMeta($resSeometaData['META_KEYWORDS'], $section, $arCondition['INFOBLOCK'], $eventRegions, $sku, $morphyObject);
                        }
                        if ($resSeometaData['META_DESCRIPTION']) {
                            $META_DESCRIPTION = self::getMeta($resSeometaData['META_DESCRIPTION'], $section, $arCondition['INFOBLOCK'], $eventRegions, $sku, $morphyObject);
                        }
                    }
                }
            }
    ...
    private static function getMeta($meta, $section, $infoblock, $eventRegions, $sku, $morphyObject)
    {
        ...
        $result = Engine::process($sku, SeoMetaMorphy::prepareForMorphy($result));
        $result = Encoding::convertEncodingToCurrent($result);
        $result = SeoMetaMorphy::convertMorphy($result, $morphyObject);

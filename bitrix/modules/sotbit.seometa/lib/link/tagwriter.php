...
class TagWriter extends AbstractWriter
{
    ...
    private function __construct($WorkingConditions, $countTagsForWrite, $elemetTag = 0)
    {
        $this->WorkingConditions = $WorkingConditions;
        $this->countTagWrite = $countTagsForWrite;
        $this->elemetTag = $elemetTag;
    }

    public static function getInstance($WorkingConditions, $countTagsForWrite, $elemetTag = 0)
    {
        if (self::$Writer === false) {
            self::$Writer = new TagWriter($WorkingConditions, $countTagsForWrite, $elemetTag);
        }
    ...
    public function Write(
        ...
        $conditionTag = $this->elemetTag ? $this->arCondition['ELEMENT_TAG'] : $this->arCondition['TAG'];
        if ($arFields['strict_relinking'] != 'Y') {
            if($this->elemetTag) {
                $Title = Engine::process($sku,SeoMetaMorphy::prepareForMorphy($this->arCondition['ELEMENT_TAG']));
            } else {

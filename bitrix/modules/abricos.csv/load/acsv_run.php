...
if (empty($arRunErrors))
{
	...
	if (empty($arRunErrors))
	{
		...
		if($info = CModule::CreateModuleObject('iblock')){
		   $d7Version = '18.6.200';
		   if(CheckVersion($d7Version, $info->MODULE_VERSION))
		      $d7=false;
		   else
		     $d7=true;
		}
		...
		if ($FILTER_PRICE_MAX)
		    {
					$filter['<PRICE_1']  = $FILTER_PRICE_MAX;
		    }
		if ($FILTER_PRICE_MIN)
		    {
					$filter['>PRICE_1']  = $FILTER_PRICE_MIN;
		    }
		...
		if($FILTER_ACTIVE!='ALL')
       	...
       	{
	        if($d7)
		        $filter['AVAILABLE'] = $FILTER_AVAILABLE;
	        else
		        $filter['CATALOG_AVAILABLE'] = $FILTER_AVAILABLE;
	    }
	    if($FILTER_QUANTYTY)
	    {
	       if($d7)
		       $filter['>QUANTITY'] = $FILTER_QUANTYTY;
	       else
		       $filter['>CATALOG_QUANTITY'] = $FILTER_QUANTYTY;
	    }
		do
		{
		...
		while ($MAX_EXECUTION_TIME == 0 && $existItems && !CAbricosCSV::getCur());
	}

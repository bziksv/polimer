...
class WriterXlsx {
	...
	function __construct($arParams = array(), $ee = false)
	{
		...
		if(file_exists($funcFile))
		{
			...
			if(!\Bitrix\EsolAie\Utils::IsUtfMode() && \CUtil::DetectUTF8($fileContent))
			{
				$fileContent = \Bitrix\Main\Text\Encoding::convertEncoding($fileContent, 'UTF-8', 'CP1251');
			}
			elseif(\Bitrix\EsolAie\Utils::IsUtfMode() && !\CUtil::DetectUTF8($fileContent))
			{
			...
			while(!feof($handle)) 
			{
				...
				else
				{
					...
					foreach($arVals as $valIndex=>$arValue)
					{
						...
						{
							$val = trim($arValue[$k]);
							if(mb_strpos($val, '=')!==0) continue;
							$isFormula = $isMathFormula = false;
							$val = mb_substr($val, 1);
							foreach($this->arFunctions as $funcCode=>$funcText)
							{
								if(mb_strpos($val, $funcText.'(')===false) continue;
								if(mb_strpos($val, $funcText.'(')===0) $isFormula = true;
		...
		if($this->byTemplate)
		{
			...
			if(file_exists($dirPath.'data/xl/sharedStrings.xml'))
			{
				...
				if($sxml->si)
				{
					...
					{
						$sharedStrings .= preg_replace_callback('/\{DATE_(\S*)\}/', array('\Bitrix\EsolAie\Export\Utils', 'GetDateFormat'), $si->asXML());
    ...
    public function openWorkSheet($listIndex, $isHidden = false)
    {
		...
		Utils::PrepareTextRows2($this->textRowsTop, $lastDataRow);
		Utils::PrepareTextRows2($this->textRowsTop2, $lastDataRow);
		Utils::PrepareTextRows2($this->textRowsTop3, $lastDataRow);
	...
	public function AddTextRows($textRows, $textRowsName, $fieldsCount)
	{
		if(!empty($textRows))
		{
			foreach($textRows as $k=>$v)
			{
				...
				/*Formula*/
				$v2 = trim($v);
				if(mb_strpos(trim($v2), '=')===0)
				{
					$isFormula = $isMathFormula = false;
					$v2 = mb_substr($v2, 1);
					foreach($this->arFunctions as $funcCode=>$funcText)
					{
						if(mb_strpos($v2, $funcText.'(')===false) continue;
						if(mb_strpos($v2, $funcText.'(')===0) $isFormula = true;
						$isMathFormula = ($isMathFormula || $this->IsMathFormula($funcCode));
	...
	public function closeWorkSheetRelsHandler()
	{
		...
		if(isset($this->workSheetRelsHandler) && is_resource($this->workSheetRelsHandler))
		{
			if($needWrite)
			{
    ...
    public function closeExcelWriter($break = false)
    {
		...
		{
			if(Utils::CanUseZipArchive() && ($zipObj = new \ZipArchive()) && $zipObj->open($this->outputFile, \ZipArchive::OVERWRITE|\ZipArchive::CREATE)===true)
			{

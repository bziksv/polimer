...
class Profile {
	...
	function __construct($entityField)
	{
		...
		$this->uploadDir = $_SERVER["DOCUMENT_ROOT"].'/upload/'.static::$moduleId.'/import/';
	...
	{
		if($entity=='false' && !empty(static::$instances))
	...
	{
		$optionName = 'DB_STRUCT_VERSION_IMPORT';
		$moduleVersion = false;
	...
	public function SetImportParams($pid, $arParams=array())
	{
		$this->pid = $pid;
	...
	public function OnEndImport($file)
	{
		$hash = md5_file($file);
		$this->UpdateFields($this->pid, array(
			'FILE_HASH'=>$hash,

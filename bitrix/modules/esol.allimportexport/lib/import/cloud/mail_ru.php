<?php
namespace Bitrix\EsolAie\Import\Cloud;
...
class MailRu
{
    ...
    /*function getDownloadToken()
    {
        ...
        if ($result !== 'error') {
			$result = \Bitrix\EsolAie\Utils::JsObjectToPhp($result);
		...
		if($client->getStatus()==200)
		{
			...
			if($hcd = $headers->get('content-disposition'))
			{
				...
				if(count($arParts) > 0)
				{
					...
					if(!\Bitrix\EsolAie\Utils::IsUtfMode() && \CUtil::DetectUTF8($fn))
					{
	...
	private function GetHttpClient($redirect = true)
	{
		...
		$client->setHeader('User-Agent', 'BitrixSM HttpClient class');
		$client->setHeader('Cookie', implode('; ', array_map(array(__CLASS__, 'GetKeyValEq'),array_keys($this->cookies), $this->cookies)));
		//$client->setCookies($this->cookies);
		return $client;
	}
	
	private static function GetKeyValEq($k, $v)
	{
		return $k."=".$v;
	}

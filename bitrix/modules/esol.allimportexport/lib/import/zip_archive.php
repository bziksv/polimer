...
class ZipArchive
{
	...
	{
		if(($pos = mb_strpos($pFilename, '/'.static::$moduleId.'/'))!==false)
		{
				...
				{					
					...
					if($rowNum > 1)
					{
						...
						if($rowNum > $lastRow)
						{
							...
							if($countMode)
							{
								$maxTime = intval(ini_get('max_execution_time')) - 10;
								if($maxTime > 0) $maxTime = 50;
								elseif($maxTime < 10) $maxTime = 15;
								elseif($maxTime > 50) $maxTime = 50;
							}
	...
	public function getSharedStrings($name, $reader, $bCache=false)
	{
		...
		else
		{
			...
			while(!feof($handle))
			{
				...
				$sharedStrings[] = \Bitrix\EsolAie\Utils::Unserialize(base64_decode($buffer));
			}
			fclose($handle);
...
class XmlUtils
{
	...
	{
		return (class_exists('\XMLReader') ? '\XMLReader' : '\Bitrix\EsolAie\Import\XMLReader');
	}

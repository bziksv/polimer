...
class Extrasettings {
	...
	public static function GetMarginTemplates(&$pfile)
	{
		...
		$arTemplates = \Bitrix\EsolAie\Utils::Unserialize(file_get_contents($pfile));
		...
		foreach($arParams as $k1=>$v1)
		{
			...
			foreach($v1 as $k2=>$v2)
			{
				...
				foreach($v2 as $k3=>$v3)
				{
					...
					if(in_array($k3, array('SET_NEW_ONLY', 'NOT_TRIM', 'INSERT_PICTURE', 'MAKE_DROPDOWN', 'STYLE_BOLD', 'STYLE_ITALIC')) && $v3!='Y') $v3 = '';

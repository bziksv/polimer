...
class Cloud
{
	...
	public function YadiskGetFileByYaLink(&$tmpPath, $fileLink)
	{
		...
		if(strlen($fileLink) > 0)
		{
			...
			if(count($filename)==1)
			{
				...
				if(!\Bitrix\EsolAie\Utils::IsUtfMode()) $filename = $GLOBALS['APPLICATION']->ConvertCharset($filename, 'UTF-8', 'CP1251');
	...
	public static function GetPatternForRegexp($pattern)
	{
		...
		$pattern = preg_replace_callback('/\\\{([^\}]*)\\\}/', array(__CLASS__, 'PathToRegexp'), $pattern);
		$pattern = strtr($pattern, array('\*'=>'.*', '\?'=>'.'));
		return '/'.$pattern.'/';
	}
	
	public static function PathToRegexp($m)
	{
		...
		if($res && $client->getStatus()!=404)
		{
			...
			if($hcd && stripos($hcd, 'filename='))
			{
				...
				if(count($hcdParts1) > 0)
				{
					...
					{
						$tmpPath = \Bitrix\EsolAie\Import\Utils::ReplaceFile($tmpPath, preg_replace('/\/[^\/]+$/', '/'.$fn, $tmpPath));
					}
				}
	...
	public function DropboxGetFile(&$tmpPath, $path, $fromFile=false)
	{
		...
		$siteEncoding = \Bitrix\EsolAie\Import\Utils::getSiteEncoding();
	...
	{
		$client = new \Bitrix\Main\Web\HttpClient(array('socketTimeout'=>10, 'disableSslVerification'=>true));
		$client->setHeader('User-Agent', \Bitrix\EsolAie\Import\Utils::GetUserAgent());
		$res = $client->get($path);

 ...
 */
	...
	private static function DisplayControl($inputs = array())
	{
		...
		if ($inputs['upload'])
		{
			self::$menuNew[] = array("ID" => "upload", "GLOBAL_ICON" => "adm-menu-upload-pc", "TEXT" => GetMessage("ESOL_AE_ADM_FILE_UPLOAD"), "CLOSE_ON_CLICK" => false);
			self::$menuExist[] = array("ID" => "upload", "GLOBAL_ICON" => "adm-menu-upload-pc", "TEXT" => GetMessage("ESOL_AE_ADM_FILE_NEW_UPLOAD"), "CLOSE_ON_CLICK" => false);
		}
		if ($inputs['medialib'])
		{
			self::$menuNew[] = array("TEXT" => GetMessage("ESOL_AE_ADM_FILE_MEDIALIB"), "GLOBAL_ICON" => "adm-menu-upload-medialib", "ONCLICK" => "OpenMedialibDialog".self::$jsId."()");
			self::$menuExist[] = array("TEXT" => GetMessage("ESOL_AE_ADM_FILE_NEW_MEDIALIB"), "GLOBAL_ICON" => "adm-menu-upload-medialib", "ONCLICK" => "OpenMedialibDialog".self::$jsId."()");
		}
		if ($inputs['file_dialog'])
		{
			self::$menuNew[] = array("TEXT" => GetMessage("ESOL_AE_ADM_FILE_SITE"), "GLOBAL_ICON" => "adm-menu-upload-site", "ONCLICK" => "OpenFileDialog".self::$jsId."()");
			self::$menuExist[] = array("TEXT" => GetMessage("ESOL_AE_ADM_FILE_NEW_SITE"), "GLOBAL_ICON" => "adm-menu-upload-site", "ONCLICK" => "OpenFileDialog".self::$jsId."()");
		}

		if ($inputs['cloud'])
		{
			self::$menuNew[] = array("TEXT" => GetMessage("ESOL_AE_ADM_FILE_CLOUD"), "GLOBAL_ICON" => "adm-menu-upload-cloud", "ONCLICK" => "OpenCloudDialog".self::$jsId."()");
			self::$menuExist[] = array("TEXT" => GetMessage("ESOL_AE_ADM_FILE_NEW_CLOUD"), "GLOBAL_ICON" => "adm-menu-upload-cloud", "ONCLICK" => "OpenCloudDialog".self::$jsId."()");
		}
		
		if ($inputs['email'])
		{
			self::$menuNew[] = array("TEXT" => GetMessage("ESOL_AE_FILE_SOURCE_EMAIL"), "GLOBAL_ICON" => "adm-menu-upload-email", "ONCLICK" => "EProfile.ShowEmailForm()");
			self::$menuExist[] = array("TEXT" => GetMessage("ESOL_AE_FILE_SOURCE_EMAIL"), "GLOBAL_ICON" => "adm-menu-upload-email", "ONCLICK" => "EProfile.ShowEmailForm()");
		}
		
		if ($inputs['linkauth'])
		{
			self::$menuNew[] = array("TEXT" => GetMessage("ESOL_AE_FILE_SOURCE_LINKAUTH"), "GLOBAL_ICON" => "adm-menu-upload-linkauth", "ONCLICK" => "EProfile.ShowFileAuthForm()");
			self::$menuExist[] = array("TEXT" => GetMessage("ESOL_AE_FILE_SOURCE_LINKAUTH"), "GLOBAL_ICON" => "adm-menu-upload-linkauth", "ONCLICK" => "EProfile.ShowFileAuthForm()");
		}
		...
		?>
		<script type="text/javascript">(top.BX.file_input) ? new top.BX.file_input(<?= \Bitrix\EsolAie\Utils::PHPToJSObject($arConfig)?>) : new BX.file_input(<?= \Bitrix\EsolAie\Utils::PHPToJSObject($arConfig)?>)</script>
		</div>
	...
	private static function DisplayFile($arFile = array(), $ind = 0)
	{
		...
		if (!$bNotFound)
		{
			...
			if ($arFile['FORMATED_SIZE'] != '')
				$hint .= '<span class="adm-input-file-hint-row">'.GetMessage('ESOL_AE_ADM_FILE_INFO_SIZE').':&nbsp;&nbsp;'.$arFile['FORMATED_SIZE'].'</span>';

			if ($arFile['IS_IMAGE'])
				$hint .= '<span class="adm-input-file-hint-row">'.GetMessage('ESOL_AE_ADM_FILE_INFO_DIM').':&nbsp;&nbsp;'.$arFile['WIDTH'].'x'.$arFile['HEIGHT'].'</span>';
			if ($sImagePath != '')
				$hint .= '<span class="adm-input-file-hint-row">'.GetMessage('ESOL_AE_ADM_FILE_INFO_LINK').':&nbsp;&nbsp;<a href="'.\CHTTP::urnEncode($sImagePath, "UTF-8").'">'.htmlspecialcharsbx($sImagePath).'</a></span>';

			if (!self::$bShowDescInput && $arFile['DESCRIPTION'] != "")
				$hint .= '<span class="adm-input-file-hint-row">'.GetMessage('ESOL_AE_ADM_FILE_DESCRIPTION').':&nbsp;&nbsp;'.htmlspecialcharsbx($arFile['DESCRIPTION']).'</span>';
		}
		...
		if ($bNotFound)
		{
			...
			<?= GetMessage('ESOL_AE_ADM_FILE_NOT_FOUND')?>
			</span>
			<?
		}
		elseif ($arFile['IS_IMAGE'])
		{
			...
			<span id="<?= $hintId?>" class="adm-input-file-preview" style="<?if(self::$minPreviewWidth > 0){echo 'min-width: '.self::$minPreviewWidth.'px;';}?> <?if(self::$minPreviewHeight > 0){echo 'min-height:'.self::$minPreviewHeight.'px;';}?>">
				...
				<div id="<?= self::$jsId.'_file_del_lbl_'.$ind?>" class="adm-input-file-del-lbl"><?= GetMessage('ESOL_AE_ADM_FILE_DELETED_TITLE')?></div>
			</span>
			<?
		}
		else
		...
		if (!$bNotFound && self::$bShowDescInput)
		{
			...
			<div id="<?= self::$jsId.'_file_desc_'.$ind?>" class="adm-input-file-desc-inp-cont" <?if($arFile['DESCRIPTION'] == ""){echo 'style="display: none;"';}?>>
				<input name="<?= $descName?>" class="adm-input" type="text" value="<?= htmlspecialcharsbx($arFile['DESCRIPTION'])?>" size="<?= self::$inputSize?>" placeholder="<?= GetMessage("ESOL_AE_ADM_FILE_DESC")?>" <?if(self::$bViewMode){echo ' disabled="disabled"';}?>>
			</div>
			<?
		}
		?>

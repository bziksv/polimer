...
class EsolIEProductTable extends \Bitrix\Sale\Internals\ProductTable
{
	...
	public static function getMap(): array
	{
		...
		if(!isset($arMap['IBLOCK_ELEMENT']))
		{
			$arMap['IBLOCK_ELEMENT'] = array(
		...
		if(array_key_exists('UF_PRODUCT_GROUP', $arUtsMap))
		{
			...
			$arMap['IE_PRODUCT_UTS.UF_PRODUCT_GROUP_TBL.UF_NAME'] = new \Bitrix\Main\Entity\StringField(
				'IE_PRODUCT_UTS.UF_PRODUCT_GROUP_TBL.UF_NAME', 
				array(
	...
	*/
	...
	public static function getIEFields($type='export')
	{
		...
		$arMap = array_diff_key($arMap, array_flip(preg_grep('/^(IE_SALEPRODUCTPROPERTY_|IE_SALEPRICE_|IE_PRODUCT_UTS)[^\.]*$/', array_keys($arMap))));
		if($type=='import')
		{
			$arMap = array_diff_key($arMap, array_flip(preg_grep('/^(IE_SALEPRODUCTPROPERTY_|IE_SALEPRICE_|IE_PRODUCT_UTS)/', array_keys($arMap))));
		}
	...
	{
		$arFields = array_diff($arFields, preg_grep('/^(IE_SALEPRODUCTPROPERTY_|IE_SALEPRICE_)/', $arFields));
	}
	...
	public static function initPriceFields()
	{
		if(!isset(self::$priceFields))
		{
			...
			{
				self::$priceFields['IE_SALEPRICE_'.$arGroup['ID']] = new \Bitrix\Main\Entity\ReferenceField(
					'IE_SALEPRICE_'.$arGroup['ID'],
				...
				self::$priceFields['IE_SALEPRICE_'.$arGroup['ID'].'.PRICE'] = new \Bitrix\Main\Entity\StringField(
					'IE_SALEPRICE_'.$arGroup['ID'].'.PRICE', 
					array(
						'title' => Loc::getMessage("ESOL_AIE_FL_PRODUCT_PRICE").' '.$arGroup['NAME']
					)
				);
				self::$priceFields['IE_SALEPRICE_'.$arGroup['ID'].'.CURRENCY'] = new \Bitrix\Main\Entity\StringField(
					'IE_SALEPRICE_'.$arGroup['ID'].'.CURRENCY', 
					array(
	...
	public static function initPropertyFields()
	{
		if(!isset(self::$propFields))
		{
			...
			while($arr = $dbRes->Fetch())
			{
				if($arr['VERSION']==1)
				{
					/*self::$propFields['IE_SALEPRODUCTPROPERTY_'.$arr['ID']] = new \Bitrix\Main\Entity\ReferenceField(
						'IE_SALEPRODUCTPROPERTY_'.$arr['ID'],
					...
					self::$propFieldTitles['IE_SALEPRODUCTPROPERTY_'.$arr['ID']] = Loc::getMessage("ESOL_AIE_FL_PRODUCT_PROPERTY").' '.$arr['NAME'].' - '.$arr['IBLOCK_NAME'].' ['.$arr['ID'].']';*/

					$addField = false;
					if($arr['PROPERTY_TYPE']=='L')
					{
						$className = 'EsolIESaleElementPropertyID'.$arr['ID'].'Table';
						...
						self::$propFields['IE_SALEPRODUCTPROPERTY_'.$arr['ID'].'.VALUE_ENUM.VALUE'] = new \Bitrix\Main\Entity\StringField(
							'IE_SALEPRODUCTPROPERTY_'.$arr['ID'].'.VALUE_ENUM.VALUE', 
							array(
					...
					elseif($arr['PROPERTY_TYPE']=='E')
					{
						...
						self::$propFields['IE_SALEPRODUCTPROPERTY_'.$arr['ID'].'.VALUE_ELEMENT.NAME'] = new \Bitrix\Main\Entity\StringField(
							'IE_SALEPRODUCTPROPERTY_'.$arr['ID'].'.VALUE_ELEMENT.NAME', 
							array(
								'title' => Loc::getMessage("ESOL_AIE_FL_PRODUCT_PROPERTY").' '.$arr['NAME'].' ['.$arr['CODE'].'] - '.$arr['IBLOCK_NAME'].' ['.$arr['IBLOCK_ID'].']'
							)
						);
					...
					elseif($arr['PROPERTY_TYPE']=='S' && $arr['USER_TYPE']=='directory' && $arr['USER_TYPE_SETTINGS_LIST']['TABLE_NAME'] && Loader::includeModule('highloadblock'))
					{
						...
						if($hlblock)
						{
							...
							if(isset($arHLFields['UF_XML_ID']) && isset($arHLFields['UF_NAME']))
							{
								...
								self::$propFields['IE_SALEPRODUCTPROPERTY_'.$arr['ID'].'.VALUE_DIR.UF_NAME'] = new \Bitrix\Main\Entity\StringField(
									'IE_SALEPRODUCTPROPERTY_'.$arr['ID'].'.VALUE_DIR.UF_NAME', 
									array(
										'title' => Loc::getMessage("ESOL_AIE_FL_PRODUCT_PROPERTY").' '.$arr['NAME'].' ['.$arr['CODE'].'] - '.$arr['IBLOCK_NAME'].' ['.$arr['IBLOCK_ID'].']'
									)
								);
					...
					if(!$addField)
					{
						if(false /*array_key_exists($arr['IBLOCK_ID'], $arProductIblocks) && $arProductIblocks[$arr['IBLOCK_ID']]['SKU_PROPERTY_ID'] > 0*/)
						{
							...
							self::$propFields['IE_SALEPRODUCTPROPERTY_'.$arr['ID'].'_SKU'] = new \Bitrix\Main\Entity\ReferenceField(
								'IE_SALEPRODUCTPROPERTY_'.$arr['ID'].'_SKU',
								'Bitrix\Iblock\EsolIEElementPropertyTable',
								array(
									'=ref.IBLOCK_ELEMENT_ID' => 'this.ID',
						...
						self::$propFields['IE_SALEPRODUCTPROPERTY_'.$arr['ID'].'.VALUE'] = new \Bitrix\Main\Entity\StringField(
							'IE_SALEPRODUCTPROPERTY_'.$arr['ID'].'.VALUE', 
							array(
								'title' => Loc::getMessage("ESOL_AIE_FL_PRODUCT_PROPERTY").' '.$arr['NAME'].' ['.$arr['CODE'].'] - '.$arr['IBLOCK_NAME'].' ['.$arr['IBLOCK_ID'].']'
							)
						);
				...
				elseif($arr['VERSION']==2)
				{
					if($arr['MULTIPLE']!='Y')
					{
						...
						if($arr['PROPERTY_TYPE']=='L')
						{
							$propSuffix = '_VALUE.VALUE';
						}
						elseif($arr['PROPERTY_TYPE']=='S' && $arr['USER_TYPE']=='directory' && $arr['USER_TYPE_SETTINGS_LIST']['TABLE_NAME'] && Loader::includeModule('highloadblock'))
						{
							$propSuffix = '_VALUE.UF_NAME';
						}

						self::$propFields['IE_SALEPRODUCTPROPTABLE_'.$arr['IBLOCK_ID'].'.PROPERTY_'.$arr['ID'].$propSuffix] = new \Bitrix\Main\Entity\StringField(
							'IE_SALEPRODUCTPROPTABLE_'.$arr['IBLOCK_ID'].'.PROPERTY_'.$arr['ID'].$propSuffix,
							array(
								'title' => Loc::getMessage("ESOL_AIE_FL_PRODUCT_PROPERTY").' '.$arr['NAME'].' ['.$arr['CODE'].'] - '.$arr['IBLOCK_NAME'].' ['.$arr['IBLOCK_ID'].']'
							)
						);
			...
			{
				$className = 'EsolIESaleElementPropertyV2Ib'.$iblockId.'Table';
				...
				$command = 'namespace Bitrix\EsolAie\Entity;'."\r\n".
					...
					'}';
				eval($command);
				self::$propFields['IE_SALEPRODUCTPROPTABLE_'.$iblockId] = new \Bitrix\Main\Entity\ReferenceField(
					'IE_SALEPRODUCTPROPTABLE_'.$iblockId,
					$fullClassName,
					array(
						'=ref.IBLOCK_ELEMENT_ID' => 'this.ID'
					)
				);
				self::$propFieldTitles['IE_SALEPRODUCTPROPTABLE_'.$iblockId] = Loc::getMessage("ESOL_AIE_FL_IBLOCK_PROPERTIES").' '.$arIblockPropsItem['IBLOCK_NAME'];
			}
		}
	}
}

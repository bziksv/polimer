...
\CJSCore::Init(array($moduleJsId.'_export'));
...
if ($REQUEST_METHOD == "POST" && $MODE=='AJAX')
{
	...
	{
		$oFilter = new \Bitrix\EsolAie\Export\Filter($entityId);
		$arValues = $oFilter->GetListValues($_POST['FIELD'], $fl);
		$APPLICATION->RestartBuffer();
		ob_end_clean();
		echo \Bitrix\EsolAie\Utils::PhpToJSObject($arValues);
		die();
	}
	...
	{
		\Bitrix\EsolAie\Export\Utils::RemoveTmpFiles();
		\Bitrix\EsolAie\Export\Utils::CheckZipArchive();
		die();
	}
...
if ($REQUEST_METHOD == "POST" && $STEP > 1 && check_bitrix_sessid())
{
	...
	if ($STEP > 1)
	{
		...
		if (strlen($strError) <= 0)
		{
			...
			if(strpos($fpath, '/')!==0)
				$strError.= Loc::getMessage("ESOL_AE_FILE_PATH_INCORRECT")."<br>";
		}
	...
	if($ACTION=='SHOW_PREVIEW')
	{
		...
		$listName = ($SETTINGS['LIST_NAME'][$list] ? $SETTINGS['LIST_NAME'][$list] : sprintf(Loc::getMessage("ESOL_AE_SHEET_NAME"), $list+1));
		
		$params = array_merge($SETTINGS_DEFAULT, $SETTINGS);
		$ee = \Bitrix\EsolAie\Export\Exporter::getInstance($entityId, $SETTINGS_DEFAULT, $SETTINGS, $EXTRASETTINGS, false, $PROFILE_ID);
		//$ee->setParams($params, $EXTRASETTINGS, false, $PROFILE_ID);
		...
		foreach($textKeys as $textKey)
		{
			...
			if(!empty($SETTINGS[$textKey][$list]))
			{
				foreach($SETTINGS[$textKey][$list] as $k=>$v)
				{
					...
					$rowContent .= '<tr title="'.sprintf(Loc::getMessage("ESOL_AE_ROW_NUMBER"), $rowIndex++).'">';
					$rowContent .= '<td><span class="sandwich" data-key="'.$dataKey.'" data-type="'.$dataType.'" title="'.Loc::getMessage("ESOL_AE_ACTIONS_BTN").'"></span></td>';
					if($dataType == 'image')
					{
						$rowContent .= '<td colspan="'.count($arFields).'"><div class="cell cell_wide80"><div class="cell_inner" '.\Bitrix\EsolAie\Export\Utils::GetCellStyleFormatted($dSettings, $SETTINGS_DEFAULT).'>';
					...
					else
					{
						...
						$rowContent .= '<textarea class="kda-ee-text-block" name="SETTINGS['.$textKey.']['.$list.']['.$k.']" '.\Bitrix\EsolAie\Export\Utils::GetCellStyleFormatted($dSettings, $SETTINGS_DEFAULT).'>'.$v.'</textarea>';
						//$rowContent .= '<input class="kda-ee-text-block-val" value="..." onclick="EList.ShowAddTextMenu(this);" type="button" title="'.Loc::getMessage("ESOL_AE_ADD_TEXT_VAL").'">';
						$rowContent .= '</div></div></td>';
					}
					$rowContent .= '</tr>';
		...
		$arSortableFields = $fl->GetSortableFields();
		$level = 0;
		...
		<div class="kda-ee-title">
			...
			<?if($list > 0){?>
				<a href="javascript:void(0)" class="kda-ee-remove-list" onclick="EList.RemoveList(this);" title="<?echo Loc::getMessage("ESOL_AE_REMOVE_LIST"); ?>"></a>
			<?}?>
		</div>
		<div class="kda-ee-hidden-settings">
			<?echo $fl->ShowSelectFields('FIELDS_LIST['.$list.']', '', $arFieldParams);?>
			<?if(isset($SETTINGS['DISPLAY_PARAMS'][$list]) && !empty($SETTINGS['DISPLAY_PARAMS'][$list])){?>
				...
				<script>EList.SetDisplayParams("<?echo $list?>", <?echo \Bitrix\EsolAie\Utils::PhpToJSObject($SETTINGS['DISPLAY_PARAMS'][$list])?>)</script>
			<?}?>
		...
		<div class="kda-ee-additional-settings">
			...
			<div class="addsettings_inner">
				<table class="additional">
					<col><col width="400px">
					<tr>
						<td><?echo Loc::getMessage("ESOL_AE_HIDE_COLUMN_TITLES"); ?>:</td>
						<td>
					...
					<tr>
						<td><?echo Loc::getMessage("ESOL_AE_ENABLE_AUTOFILTER"); ?>:</td>
						<td>
					...
					<tr>
						<td><?echo Loc::getMessage("ESOL_AE_ENABLE_PROTECTION"); ?>:</td>
						<td>
		...
		echo '<th title="'.sprintf(Loc::getMessage("ESOL_AE_ROW_NUMBER"), $rowTitleIndex).'"><span class="sandwich" data-key="COLUMN_TITLES" title="'.Loc::getMessage("ESOL_AE_ACTIONS_BTN").'"></span></th>';
		foreach($arFields as $k=>$field)
		{
			...
			{
				$sortClass = 'sort_up';
				$sortTitle = Loc::getMessage("ESOL_AE_SETTINGS_SORT_ASC");
				if($sortBy==$field)
				{
					if($sortOrder!='DESC')
					{
						$sortClass = 'sort_down';
						$sortTitle = Loc::getMessage("ESOL_AE_SETTINGS_SORTED_ASC")."\r\n".Loc::getMessage("ESOL_AE_SETTINGS_SORT_DESC");
					}
					else
					{
						$sortTitle = Loc::getMessage("ESOL_AE_SETTINGS_SORTED_DESC")."\r\n".$sortTitle;
					}
					$sortClass .= ' active';
				}
			}
			echo '<th>'.
					'<div>'.
						...
						'<a href="javascript:void(0)" class="field_settings '.(empty($EXTRASETTINGS[$list][$k]) ? 'inactive' : '').'" id="field_settings_'.$list.'_'.$k.'" title="'.Loc::getMessage("ESOL_AE_SETTINGS_FIELD").'" onclick="EList.ShowFieldSettings(this);">'.
							'<input type="hidden" name="EXTRASETTINGS['.$list.']['.$k.']" value="">'.
							'<script>EList.SetExtraParams("field_settings_'.$list.'_'.$k.'", '.(empty($EXTRASETTINGS[$list][$k]) ? '""' : \Bitrix\EsolAie\Utils::PhpToJSObject($EXTRASETTINGS[$list][$k])).')</script>'.
						'</a>'.
						'<a href="javascript:void(0)" class="field_delete" title="'.Loc::getMessage("ESOL_AE_SETTINGS_DELETE_FIELD").'" onclick="EList.DeleteColumn(this);"></a>'.
						'<a href="javascript:void(0)" onclick="EList.AddColumn(this);" class="kda-ee-new-column" title="'.Loc::getMessage("ESOL_AE_SETTINGS_ADD_FIELD").'"></a>'.
					'</div>'.
					'<div>'.
						'<input type="text" name="SETTINGS[FIELDS_LIST_NAMES]['.$list.']['.$k.']" value="'.htmlspecialcharsbx($fieldName).'" class="fieldname" '.\Bitrix\EsolAie\Export\Utils::GetCellStyleFormatted((isset($SETTINGS['DISPLAY_PARAMS'][$list]['COLUMN_TITLES']) ? $SETTINGS['DISPLAY_PARAMS'][$list]['COLUMN_TITLES'] : array()), $SETTINGS_DEFAULT).'>'.
		...
		foreach($arData as $arElement)
		{
			...
			if(isset($arElement['RTYPE']) && ($arElement['RTYPE']=='SECTION_PATH' || preg_match('/^SECTION_(\d+)$/', $arElement['RTYPE'], $m)))
			{
				...
				echo '<td title="'.sprintf(Loc::getMessage("ESOL_AE_ROW_NUMBER"), $rowIndex++).'"><span class="sandwich" data-key="'.$arElement['RTYPE'].'" title="'.Loc::getMessage("ESOL_AE_ACTIONS_BTN").'"></span></td>';
				echo '<td colspan="'.count($arFields).'"><div class="cell cell_wide"><div class="cell_inner" '.\Bitrix\EsolAie\Export\Utils::GetCellStyleFormatted($SETTINGS['DISPLAY_PARAMS'][$list][$arElement['RTYPE']], $SETTINGS_DEFAULT).'>'.$arElement['NAME'].'</div></div></td>';
			}
			else
			{
				echo '<td title="'.sprintf(Loc::getMessage("ESOL_AE_ROW_NUMBER"), $rowIndex++).'"></td>';
				foreach($arFields as $key=>$field)
				{
					$val = (array_key_exists($field.'_'.$key, $arElement) ? $arElement[$field.'_'.$key] : $arElement[$field]);
					$fSettings = $EXTRASETTINGS[$list][$key];
					if(isset($fSettings['INSERT_PICTURE']) && $fSettings['INSERT_PICTURE']=='Y' && $ee->IsPictureField($field) && ((is_array($val) && !empty($val)) || (!is_array($val) && strlen($val) > 0)))
					{
						...
						elseif(is_array($val) && $val['TYPE']=='MULTICELL')
						{
							foreach($val as $mkey=>$mval)
							{
								...
								if(preg_match('/(<a[^>]+class="kda\-ee\-conversion\-link"[^>]*>)(.*)(<\/a>)/Uis', $mval, $m))
								{
									$val[$mkey] = $m[1].'<img src="'.htmlspecialcharsbx($imageDir.$m[2]).'?hash='.mt_rand().'" style="max-width: '.$maxWidth.'px; max-height: '.$maxHeight.'px;">'.$m[3];
								}
								else
								{
									$val[$mkey] = '<img src="'.htmlspecialcharsbx($imageDir.$mval).'?hash='.mt_rand().'" style="max-width: '.$maxWidth.'px; max-height: '.$maxHeight.'px;">';
								}
							}
						}
						else
					...
					if(is_array($val) && isset($val['TYPE']) && $val['TYPE']=='MULTICELL')
					{
						...
						foreach($val as $kVal=>$vVal)
						{
							...
							$style = (is_array($vVal) && isset($vVal['STYLE']) ? \Bitrix\EsolAie\Export\Utils::GetCellStyleFormatted(array_merge($fSettings, $vVal['STYLE']), $SETTINGS_DEFAULT) : '');
					...
					echo '<td><div class="cell"><div class="cell_inner"'.($SETTINGS_DEFAULT['NOT_SHOW_PREVIEW_STYLES']=='Y' ? '' : ' '.\Bitrix\EsolAie\Export\Utils::GetCellStyleFormatted($fSettings, $SETTINGS_DEFAULT)).'>'.nl2br($val).'</div></div></td>';
				}
			}
			echo '</tr>';
		}
	...
	if($ACTION == 'DO_EXPORT')
	{
		...
		$arResult = $moduleRunnerClass::DoExport($entityId, $params, $EXTRASETTINGS, $stepparams, $PROFILE_ID);
		$APPLICATION->RestartBuffer();
		ob_end_clean();
...
<form method="POST" action="?<?if(strlen($PROFILE_ID) > 0){echo 'PROFILE_ID='.htmlspecialcharsbx($PROFILE_ID).'&';}?>lang=<?echo LANG ?>&entity=<?echo $entityId?>" ENCTYPE="multipart/form-data" name="dataload" id="dataload" class="kda-ee-s1-form">
...
?>
	...
	<tr class="heading">
		<td colspan="2" class="kda-ee-profile-header">
			<div>
				...
				<?/*?><a href="javascript:void(0)" onclick="EHelper.ShowHelp();" title="<?echo Loc::getMessage("ESOL_AE_MENU_HELP"); ?>" class="kda-ee-help-link"></a><?*/?>
			</div>
		</td>
	</tr>
	...
	?>
		...
		<tr>
			...
			$xlsxShow = true;
			?>
			<td width="40%"><?echo Loc::getMessage("ESOL_AE_FILE_EXT"); ?></td>
		...
		<tr>
			<td><?echo Loc::getMessage("ESOL_AE_FILE_PATH"); ?>:  </td>
			<td>
				<?
		...
		<?/*?>
		<tr>
			<td><?echo Loc::getMessage("ESOL_AE_MAX_SECTION_LEVEL"); ?>:  <span id="hint_MAX_SECTION_LEVEL"></span><script>BX.hint_replace(BX('hint_MAX_SECTION_LEVEL'), '<?echo Loc::getMessage("ESOL_AE_MAX_SECTION_LEVEL_HINT"); ?>');</script></td>
			<td>
		...
		<tr>
			<td><?echo Loc::getMessage("ESOL_AE_EXPORT_FIELD_CODES"); ?>:</td>
			<td>
		...
		<tr>
			<td><?echo Loc::getMessage("ESOL_AE_EXPORT_GROUP_BY_BASE_TABLE"); ?>:</td>
			<td>
				<input id="ea_group_bbt" type="checkbox" name="SETTINGS_DEFAULT[GROUP_BY_BASE_TABLE]" value="Y" <?if($SETTINGS_DEFAULT['GROUP_BY_BASE_TABLE']=='Y'){echo 'checked';}?> onclick="if(this.checked){document.getElementById('ea_merge_cells_bbt').checked = false;}">
			</td>
		</tr>
		<tr>
			<td><?echo Loc::getMessage("ESOL_AE_EXPORT_MERGE_CELLS_BY_BASE_TABLE"); ?>:</td>
			<td>
				<input id="ea_merge_cells_bbt" type="checkbox" name="SETTINGS_DEFAULT[MERGE_CELLS_BY_BASE_TABLE]" value="Y" <?if($SETTINGS_DEFAULT['MERGE_CELLS_BY_BASE_TABLE']=='Y'){echo 'checked';}?> onclick="if(this.checked){document.getElementById('ea_group_bbt').checked = false;}">
			</td>
		</tr>
		
		<tr class="heading" id="csv_settings_block">
			<td colspan="2"><?echo Loc::getMessage("ESOL_AE_SETTINGS_CSV"); ?></td>
		</tr>
		...
		?>
		<tr>
			<td><?echo Loc::getMessage("ESOL_AE_CSV_SEPARATOR"); ?>:</td>
			<td>
				<select name="SETTINGS_DEFAULT[CSV_SEPARATOR]">
					<option value=";" <?if($SETTINGS_DEFAULT['CSV_SEPARATOR']==';'){echo 'selected';}?>><?echo Loc::getMessage("ESOL_AE_CSV_SEPARATOR_SEMICOLON"); ?></option>
					<option value="," <?if($SETTINGS_DEFAULT['CSV_SEPARATOR']==','){echo 'selected';}?>><?echo Loc::getMessage("ESOL_AE_CSV_SEPARATOR_COMMA"); ?></option>
					<option value="\t" <?if($SETTINGS_DEFAULT['CSV_SEPARATOR']=='\t'){echo 'selected';}?>><?echo Loc::getMessage("ESOL_AE_CSV_SEPARATOR_TAB"); ?></option>
				</select>
			</td>
		</tr>
		
		<tr>
			<td><?echo Loc::getMessage("ESOL_AE_CSV_ENCLOSURE"); ?>:</td>
			<td>
				<select name="SETTINGS_DEFAULT[CSV_ENCLOSURE]" <?if($csvYandex){echo 'disabled';}?>>
					<option value='"' <?if($SETTINGS_DEFAULT['CSV_ENCLOSURE']=='"'){echo 'selected';}?>><?echo Loc::getMessage("ESOL_AE_CSV_ENCLOSURE_DOUBLE_QUOTE"); ?></option>
					<option value="'" <?if($SETTINGS_DEFAULT['CSV_ENCLOSURE']=="'"){echo 'selected';}?>><?echo Loc::getMessage("ESOL_AE_CSV_ENCLOSURE_SINGLE_QUOTE"); ?></option>
					<option value="" <?if($SETTINGS_DEFAULT['CSV_ENCLOSURE']===""){echo 'selected';}?>><?echo Loc::getMessage("ESOL_AE_CSV_ENCLOSURE_EMPTY"); ?></option>
				</select>
		...
		<tr>
			<td><?echo Loc::getMessage("ESOL_AE_CSV_ENCODING"); ?>:</td>
			<td>
		...
		<?/*?><tr>
			<td><?echo Loc::getMessage("ESOL_AE_CSV_YANDEX"); ?>:</td>
			<td>
		...
		<?/*?>
		<tr class="heading">
			<td colspan="2"><?echo Loc::getMessage("ESOL_AE_SETTINGS_DISPLAY"); ?> <a href="javascript:void(0)" onclick="EProfile.ToggleAdditionalSettings(this)" class="kda-head-more show"><?echo Loc::getMessage("ESOL_AE_SETTINGS_ADDITONAL_SHOW_HIDE"); ?></a></td>
		</tr>
		
		<tr>
			<td><?echo Loc::getMessage("ESOL_AE_DISPLAY_LOCK_HEADERS"); ?>:</td>
			<td>
		...
		<tr>
			<td><?echo Loc::getMessage("ESOL_AE_EXPORT_ROW_AUTO_HEIGHT"); ?>:</td>
			<td>
		...
		<tr>
			<td><?echo Loc::getMessage("ESOL_AE_EXPORT_ROW_MIN_HEIGHT"); ?>:</td>
			<td>
		...
		<tr>
			<td><?echo Loc::getMessage("ESOL_AE_DISPLAY_TEXT_ALIGN"); ?>:</td>
			<td>
				<select name="SETTINGS_DEFAULT[DISPLAY_TEXT_ALIGN]">
					<option value="LEFT" <?if($SETTINGS_DEFAULT['DISPLAY_TEXT_ALIGN']=='LEFT'){echo 'selected';}?>><?echo Loc::getMessage("ESOL_AE_DISPLAY_TEXT_ALIGN_LEFT"); ?></option>
					<option value="CENTER" <?if($SETTINGS_DEFAULT['DISPLAY_TEXT_ALIGN']=='CENTER'){echo 'selected';}?>><?echo Loc::getMessage("ESOL_AE_DISPLAY_TEXT_ALIGN_CENTER"); ?></option>
					<option value="RIGHT" <?if($SETTINGS_DEFAULT['DISPLAY_TEXT_ALIGN']=='RIGHT'){echo 'selected';}?>><?echo Loc::getMessage("ESOL_AE_DISPLAY_TEXT_ALIGN_RIGHT"); ?></option>
				</select>
			</td>
		</tr>
		
		<tr>
			<td><?echo Loc::getMessage("ESOL_AE_DISPLAY_VERTICAL_ALIGN"); ?>:</td>
			<td>
				<select name="SETTINGS_DEFAULT[DISPLAY_VERTICAL_ALIGN]">
					<option value="TOP" <?if($SETTINGS_DEFAULT['DISPLAY_VERTICAL_ALIGN']=='TOP'){echo 'selected';}?>><?echo Loc::getMessage("ESOL_AE_DISPLAY_VERTICAL_ALIGN_TOP"); ?></option>
					<option value="CENTER" <?if($SETTINGS_DEFAULT['DISPLAY_VERTICAL_ALIGN']=='CENTER'){echo 'selected';}?>><?echo Loc::getMessage("ESOL_AE_DISPLAY_VERTICAL_ALIGN_CENTER"); ?></option>
					<option value="BOTTOM" <?if($SETTINGS_DEFAULT['DISPLAY_VERTICAL_ALIGN']=='BOTTOM'){echo 'selected';}?>><?echo Loc::getMessage("ESOL_AE_DISPLAY_VERTICAL_ALIGN_BOTTOM"); ?></option>
				</select>
			</td>
		</tr>
		
		<tr>
			<td><?echo Loc::getMessage("ESOL_AE_DISPLAY_SETTING_FONT_FAMILY");?>:</td>
			<td>
		...
		<tr>
			<td><?echo Loc::getMessage("ESOL_AE_DISPLAY_SETTING_FONT_SIZE");?>:</td>
			<td>
		...
		<tr>
			<td><?echo Loc::getMessage("ESOL_AE_DISPLAY_SETTING_FONT_COLOR");?>:</td>
			<td>
		...
		<tr>
			<td><?echo Loc::getMessage("ESOL_AE_DISPLAY_SETTING_FONT_STYLE_BOLD");?>:</td>
			<td>
		...
		<tr>
			<td><?echo Loc::getMessage("ESOL_AE_DISPLAY_SETTING_FONT_STYLE_ITALIC");?>:</td>
			<td>
		...
		<tr>
			<td><?echo Loc::getMessage("ESOL_AE_DISPLAY_SETTING_BORDER_STYLE");?>:</td>
			<td>
				<select name="SETTINGS_DEFAULT[BORDER_STYLE]">
					<option value="NONE" <?if($SETTINGS_DEFAULT['BORDER_STYLE']=='NONE'){echo 'selected';}?>><?echo Loc::getMessage("ESOL_AE_DISPLAY_SETTING_BORDER_STYLE_NONE"); ?></option>
					<option value="THIN" <?if($SETTINGS_DEFAULT['BORDER_STYLE']=='THIN'){echo 'selected';}?>><?echo Loc::getMessage("ESOL_AE_DISPLAY_SETTING_BORDER_STYLE_THIN"); ?></option>
					<option value="MEDIUM" <?if($SETTINGS_DEFAULT['BORDER_STYLE']=='MEDIUM'){echo 'selected';}?>><?echo Loc::getMessage("ESOL_AE_DISPLAY_SETTING_BORDER_STYLE_MEDIUM"); ?></option>
					<option value="THICK" <?if($SETTINGS_DEFAULT['BORDER_STYLE']=='THICK'){echo 'selected';}?>><?echo Loc::getMessage("ESOL_AE_DISPLAY_SETTING_BORDER_STYLE_THICK"); ?></option>
				</select>
			</td>
		</tr>
		
		<tr>
			<td><?echo Loc::getMessage("ESOL_AE_DISPLAY_SETTING_BORDER_COLOR");?>:</td>
			<td>
		...
		<tr class="heading">
			<td colspan="2"><?echo GetMessage("ESOL_AE_EXT_SERVICES"); ?> <a href="javascript:void(0)" onclick="EProfile.ToggleAdditionalSettings(this)" class="kda-head-more show"><?echo GetMessage("ESOL_AE_SETTINGS_ADDITONAL_SHOW_HIDE"); ?></a></td>
		</tr>
		
		<tr>
			<td><?echo Loc::getMessage("ESOL_AE_OTHER_SETTINGS_UPLOAD_ON_FTP");?>:</td>
			<td>
				<input type="text" name="SETTINGS_DEFAULT[UPLOAD_ON_FTP]" value="<?=htmlspecialcharsex($SETTINGS_DEFAULT['UPLOAD_ON_FTP'])?>" placeholder="<?echo Loc::getMessage("ESOL_AE_OTHER_SETTINGS_UPLOAD_ON_FTP_EXAMPLE");?>" size="55">
			</td>
		</tr>
		
		<tr>
			<td><?echo GetMessage("ESOL_AE_EXT_SERVICES_BX24");?>:</td>
			<td>
				...
				<td id="bx24p"<?if($SETTINGS_DEFAULT['EXPORT_TO_BX24']!='Y'){echo ' style="display: none;"';}?>>
					<div id="bx24_help" style="display: none;">
						...
						<?echo sprintf(GetMessage("ESOL_AE_EXT_SERVICES_BX24_REST_URL_HELP"), sprintf($imgStr, 'bx24_1.jpg'), sprintf($imgStr, 'bx24_2.jpg'), sprintf($imgStr, 'bx24_3.jpg'), sprintf($imgStr, 'bx24_4.jpg'));?>
					</div>
					<div>
						<?echo GetMessage("ESOL_AE_EXT_SERVICES_BX24_REST_URL");?>:
						...
						<?echo sprintf(GetMessage("ESOL_AE_EXT_SERVICES_BX24_REST_URL_EXAMPLE"), 'https://b24-b41cc2.bitrix24.ru/rest/1/e9pzrkwg0aijfh9m/');?> &nbsp; (<a href="javascript:void(0)" id="bx24_help_link"><?echo GetMessage("ESOL_AE_EXT_SERVICES_BX24_REST_URL_HOW_GET");?></a>)
					</div>
					<br>
					<div>
						<?echo GetMessage("ESOL_AE_EXT_SERVICES_BX24_FOLDER_ID");?>:
						<div>
					...
					<br>
					<div>
						<?echo GetMessage("ESOL_AE_EXT_SERVICES_BX24_MODE");?>:
						<div>
							<input type="radio" name="SETTINGS_DEFAULT[BX24_MODE]" value="UPDATE"<?if($SETTINGS_DEFAULT['BX24_MODE']!='REPLACE'){echo ' checked';}?> id="bx24_mode_update"> <label for="bx24_mode_update"><?echo GetMessage("ESOL_AE_EXT_SERVICES_BX24_MODE_UPDATE");?></label>
							<br>
							<input type="radio" name="SETTINGS_DEFAULT[BX24_MODE]" value="REPLACE"<?if($SETTINGS_DEFAULT['BX24_MODE']=='REPLACE'){echo ' checked';}?> id="bx24_mode_replace"> <label for="bx24_mode_replace"><?echo GetMessage("ESOL_AE_EXT_SERVICES_BX24_MODE_REPLACE");?></label>
						</div>
					</div>
				</td>
				</tr></table>
		...
		<tr>
			<td><?echo GetMessage("ESOL_AE_EXT_SERVICES_GOOGLE_SPREADSHEETS");?>:</td>
			<td>
				<table cellspacing="0"><tr>
				...
				<td id="googless"<?if($SETTINGS_DEFAULT['EXPORT_TO_GOOGLE_SPREADSHEETS']!='Y'){echo ' style="display: none;"';}?>>
					<div style="margin-bottom: 7px;">
						<?echo GetMessage("ESOL_AE_EXT_SERVICES_GOOGLE_TOKEN");?>:
						<div><input type="text" name="SETTINGS_DEFAULT[GOOGLE_TOKEN]" value="<?=htmlspecialcharsbx($SETTINGS_DEFAULT['GOOGLE_TOKEN'])?>" size="50"> &nbsp; 
						
						<a href="https://esolutions.su/marketplace/oauth.php?get_token=1" target="_blank"><?echo GetMessage("ESOL_AE_EXT_SERVICES_GOOGLE_TOKEN_GET");?></a> <?/*echo GetMessage("ESOL_AE_EXT_SERVICES_GOOGLE_TOKEN_GET_MORE");*/?>
						
						</div>
					</div>
					<div>
						<?echo GetMessage("ESOL_AE_EXT_SERVICES_GOOGLE_SID");?>:
		...
		<tr>
			<td><?echo GetMessage("ESOL_AE_EXT_SERVICES_EMAIL");?>:</td>
			<td>
				<table cellspacing="0"><tr>
				...
				<td id="sendemail"<?if($SETTINGS_DEFAULT['EXPORT_TO_EMAIL']!='Y'){echo ' style="display: none;"';}?>>
					<?$eventName = 'ESOL_ALLEXPORT_SEND_FILE';?>
					<div>
						<?echo sprintf(GetMessage("ESOL_AE_EXT_SERVICES_EMAIL_TEMPLATE"), $eventName);?>:
						<div style="padding-top: 2px;"><select name="SETTINGS_DEFAULT[MAIL_TEMPLATE_ID]" style="width: 420px;">
							<option value=""><?echo GetMessage("ESOL_AE_NOT_CHOOSE");?></option>
						<?
					...
					<div>
						<?echo sprintf(GetMessage("ESOL_AE_EXT_SERVICES_EMAIL_RECIPIENT"), $eventToField);?>:
		...
		<tr class="heading">
			<td colspan="2"><?echo Loc::getMessage("ESOL_AE_OTHER_SETTINGS"); ?> <a href="javascript:void(0)" onclick="EProfile.ToggleAdditionalSettings(this)" class="kda-head-more show"><?echo Loc::getMessage("ESOL_AE_SETTINGS_ADDITONAL_SHOW_HIDE"); ?></a></td>
		</tr>
		
		<?if(\Cmodule::IncludeModule('fileman')){?>
		<tr>
			<td><?echo GetMessage("ESOL_AE_DISPLAY_SETTING_TEMPLATE_FILE");?>: <span id="hint_TEMPLATE_FILE"></span><script>BX.hint_replace(BX('hint_TEMPLATE_FILE'), '<?echo GetMessage("ESOL_AE_DISPLAY_SETTING_TEMPLATE_FILE_HINT"); ?>');</script></td>
		...
		<tr>
			<td><?echo GetMessage("ESOL_AE_OTHER_USE_NEW_FILTER");?>: <span id="hint_USE_NEW_FILTER"></span><script>BX.hint_replace(BX('hint_USE_NEW_FILTER'), '<?echo GetMessage("ESOL_AE_OTHER_USE_NEW_FILTER_HINT"); ?>');</script></td>
			<td>
		...
		<?/*?>
		<tr>
			<td><?echo Loc::getMessage("ESOL_AE_OTHER_EXPORT_FILES_IN_ARCHIVE");?>:</td>
			<td>
				<table cellspacing="0"><tr>
				<td style="padding-left: 0px;">
					<input type="checkbox" name="SETTINGS_DEFAULT[EXPORT_FILES_IN_ARCHIVE]" value="Y" <?=htmlspecialcharsex($SETTINGS_DEFAULT['EXPORT_FILES_IN_ARCHIVE']=='Y' ? 'checked' : '')?> onchange="if(this.checked){$('#EXPORT_FILES_IN_ARCHIVE_wrap').show();}else{$('#EXPORT_FILES_IN_ARCHIVE_wrap').hide();}">
				</td>
				<td>&nbsp; &nbsp;</td>
				<td>
					<div id="EXPORT_FILES_IN_ARCHIVE_wrap" <?if($SETTINGS_DEFAULT['EXPORT_FILES_IN_ARCHIVE']!='Y'){echo 'style="display: none;"';}?>>
						<?echo Loc::getMessage("ESOL_AE_OTHER_FILES_ARCHIVE_PATH");?><br>
						<?
		...
		<tr>
			<td><?echo Loc::getMessage("ESOL_AE_OTHER_SETTINGS_MERGE_SHEETS");?>:</td>
			<td>
		...
		<tr>
			<td><?echo Loc::getMessage("ESOL_AE_OTHER_SETTINGS_NOT_SHOW_PREVIEW_STYLES");?>: <span id="hint_NOT_SHOW_PREVIEW_STYLES"></span><script>BX.hint_replace(BX('hint_NOT_SHOW_PREVIEW_STYLES'), '<?echo Loc::getMessage("ESOL_AE_OTHER_SETTINGS_NOT_SHOW_PREVIEW_STYLES_HINT"); ?>');</script></td>
			<td>
		...
		<tr>
			<td><?echo Loc::getMessage("ESOL_AE_OTHER_SETTINGS_COUNT_SHOW_LINES");?>:</td>
			<td>
	...
	<tr>
		<td colspan="2" id="kda-ee-sheet-list">
				...
				<div class="kda-ee-sheet-wrap<?if($ind > 0){echo ' withmargin';}?>">
					<?
						if($SETTINGS_DEFAULT['USE_NEW_FILTER']=='Y')
						{
							...
							if(!is_array($SETTINGS)) $SETTINGS = array();
							?>
							<!--div class="kda-ee-sheet-filter-cnt"><? echo sprintf(GetMessage('ESOL_AE_PF_FILTER_COUNT'), (isset($arEtypes[$arTemplate['ETYPE']]) ? $arEtypes[$arTemplate['ETYPE']]['NAME'] : '')); ?>: <b></b></div>-->
							
							<div class="kda-ee-sheet-cfilter-wrap">
								<div class="kda-ee-sheet-cfilter-title-wrap">
									<span class="kda-ee-sheet-cfilter-title kda-ee-sheet-cfilter-title-active" onclick="EProfile.ChangeFilter(this)"><?echo GetMessage('ESOL_AE_PF_FILTER_ELEMENTS')?></span>
								</div>
								<div class="kda-ee-sheet-cfilter-wrap-inner">
									<?								
									$eFilter = new \Bitrix\EsolAie\Export\Filter($entityId);
									...
									<div class="kda-ee-sheet-cfilter-bottom">
										<span class="adm-btn-wrap"><input type="submit" class="adm-btn" name="set_filter" value="<?echo GetMessage('ESOL_AE_PF_FILTER_APPLY')?>" onclick="return EList.ApplyFilter(this);"></span>
									</div>
								</div>
							</div>
							<?
						...
						{
							$filterId = 'esol_allimportexport_'.$PROFILE_ID.'_'.$list;
							\Bitrix\EsolAie\Export\Utils::ShowFilter($fl->GetEntityFieldsForFilter(), $filterId, $list, $SETTINGS, $SETTINGS_DEFAULT);
						}
					...
					<div class="kda-ee-new-list-wrap" style="display: none;">
						<input type="button" value="<?echo Loc::getMessage("ESOL_AE_ADD_NEW_LIST"); ?>" title="<?echo Loc::getMessage("ESOL_AE_ADD_NEW_LIST_HINT");?>">
					</div>
				</div>
				<?
				$ind++;
			...
			<?/*?><div class="kda-ee-new-list-wrap">
				<input type="button" value="<?echo Loc::getMessage("ESOL_AE_ADD_NEW_LIST"); ?>" onclick="EList.AddNewList(this);">
			</div><?*/?>
		</td>
	</tr>
		...
		<td id="resblock" class="kda-ee-result">
		 <table width="100%"><tr><td width="50%">
			<div id="progressbar"><span class="pline"></span><span class="presult load"><b>0%</b><span 
				...
				data-export="<?echo Loc::getMessage("ESOL_AE_STATUS_EXPORT"); ?>" 
			><?echo Loc::getMessage("ESOL_AE_EXPORT_INIT"); ?></span></span></div>

			<div id="block_error_import" style="display: none;">
				<?echo CAdminMessage::ShowMessage(array(
					"TYPE" => "ERROR",
					"MESSAGE" => Loc::getMessage("ESOL_AE_IMPORT_ERROR_CONNECT"),
					/*"DETAILS" => '<div><a href="javascript:void(0)" onclick="EProfile.ContinueProccess(this, '.$PROFILE_ID.');">'.Loc::getMessage("ESOL_AE_PROCESSED_CONTINUE").'</a><br><br>'.sprintf(Loc::getMessage("ESOL_AE_IMPORT_ERROR_CONNECT_COMMENT"), '/bitrix/admin/settings.php?lang=ru&mid='.$moduleId.'&mid_menu=1').'</div>',*/
					"DETAILS" => '<div>'.(COption::GetOptionString($moduleId, 'AUTO_CONTINUE_EXPORT', 'N')=='Y' ? sprintf(Loc::getMessage("ESOL_AE_EXPORT_AUTO_CONTINUE"), '<span id="kda_ee_auto_continue_time"></span>').'<br>' : '').'<a href="javascript:void(0)" onclick="EProfile.ContinueProccess(this, '.$PROFILE_ID.');">'.Loc::getMessage("ESOL_AE_PROCESSED_CONTINUE").'</a></div>',
					"HTML" => true,
				))?>
			</div>
			...
			<div class="detail_status">
				<?
				$outputFile = \Bitrix\EsolAie\Export\Utils::PrepareExportFileName($SETTINGS_DEFAULT['FILE_PATH']);
				...
				echo CAdminMessage::ShowMessage(array(
					...
					Loc::getMessage("ESOL_AE_SU_ALL").' <b id="total_read_line">0</b><br>'.
					/*.Loc::getMessage("ESOL_AE_SU_ELEMENT_ADDED").' <b id="element_added_line">0</b><br>'.
					(!empty($SETTINGS_DEFAULT['ELEMENT_UID_SKU']) ? (Loc::getMessage("ESOL_AE_SU_SKU_ADDED").' <b id="sku_added_line">0</b><br>') : '').
					Loc::getMessage("ESOL_AE_SU_SECTION_ADDED").' <b id="section_added_line">0</b><br>'.*/
					' <span id="kda_ee_ready_file" style="visibility: hidden;"><br>'.Loc::getMessage("ESOL_AE_READY_FILE_LINK").' <br><a href="'.htmlspecialcharsex($outputFile).'?hash='.md5(mt_rand()).'" target="_blank">'.$outputFile.'</a> <br><br><a href="/bitrix/admin/fileman_file_download.php?path='.htmlspecialcharsex($outputFile).'&lang='.LANGUAGE_ID.'&hash='.md5(mt_rand()).'">'.Loc::getMessage("ESOL_AE_DOWNLOAD_FILE").'</a></span><br>',
					"HTML" => true,
				));
				?>
			</div>
...
<input type="hidden" name="ENTITY" value="<?echo $entityId;?>">
<?
if($STEP > 1)
{
	if(strlen($PROFILE_ID) > 0)

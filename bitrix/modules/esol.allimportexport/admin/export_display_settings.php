...
function GetFieldEextraVal($arSettings, $fieldPrefix, $name)
{
	$fName = $fieldPrefix.'['.$name.']';
	if(preg_match_all('/\[([^\]]*)\]/Us', $fieldPrefix, $m))
	{
...
if($_POST['action']=='save' && is_array($_POST['SETTINGS']))
{
	...
	ob_end_clean();
	$returnJson = (empty($_POST['SETTINGS']['DISPLAY_PARAMS'][$listIndex]) ? '""' : \Bitrix\EsolAie\Utils::PhpToJSObject($_POST['SETTINGS']['DISPLAY_PARAMS'][$listIndex]));
	...
	<table width="100%">
		...
		<?if($fieldKey=='SECTION_PATH'){?>
			<tr>
				...
				<td class="adm-detail-content-cell-r">
					<?
					list($fName, $val) = GetFieldEextraVal($SETTINGS, $fieldPrefix, 'SECTION_PATH_SEPARATOR');
					?>
		...
		<?if(strpos($fieldKey, 'TEXT_ROWS_TOP')===0){?>
			<tr>
				...
				<td class="adm-detail-content-cell-r">
					<?
					list($fName, $val) = GetFieldEextraVal($SETTINGS, $fieldPrefix, 'HIDE_UNDER_GROUP');
					?>
		...
		<tr>
			...
			<td class="adm-detail-content-cell-r">
				<?
				list($fName, $val) = GetFieldEextraVal($SETTINGS, $fieldPrefix, 'TEXT_ALIGN');
		...
		<tr>
			...
			<td class="adm-detail-content-cell-r">
				<?
				list($fName, $val) = GetFieldEextraVal($SETTINGS, $fieldPrefix, 'VERTICAL_ALIGN');
		...
		<tr>
			...
			<td class="adm-detail-content-cell-r">
				<?
				list($fName, $val) = GetFieldEextraVal($SETTINGS, $fieldPrefix, 'ROW_HEIGHT');
				?>
		...
		<?if($fieldType!='image'){?>
			<tr>
				...
				<td class="adm-detail-content-cell-r">
					<?
					list($fName, $val) = GetFieldEextraVal($SETTINGS, $fieldPrefix, 'FONT_FAMILY');
					?>
			...
			<tr>
				...
				<td class="adm-detail-content-cell-r">
					<?
					list($fName, $val) = GetFieldEextraVal($SETTINGS, $fieldPrefix, 'FONT_SIZE');
					?>
			...
			<tr>
				...
				<td class="adm-detail-content-cell-r">
					<?
					list($fName, $val) = GetFieldEextraVal($SETTINGS, $fieldPrefix, 'FONT_COLOR');
					?>
			...
			<tr>
				...
				<td class="adm-detail-content-cell-r">
					<?
					list($fName, $val) = GetFieldEextraVal($SETTINGS, $fieldPrefix, 'BACKGROUND_COLOR');
					?>
			...
			<tr>
				...
				<td class="adm-detail-content-cell-r">
					<?
					list($fName, $val) = GetFieldEextraVal($SETTINGS, $fieldPrefix, 'STYLE_BOLD');
					?>
					<input type="hidden" name="<?=$fName?>" value="N">
					<input type="checkbox" name="<?=$fName?>" value="Y" <?if($val=='Y'){echo 'checked';}?>>
				</td>
			...
			<tr>
				...
				<td class="adm-detail-content-cell-r">
					<?
					list($fName, $val) = GetFieldEextraVal($SETTINGS, $fieldPrefix, 'STYLE_ITALIC');
					?>
					<input type="hidden" name="<?=$fName?>" value="N">
					<input type="checkbox" name="<?=$fName?>" value="Y" <?if($val=='Y'){echo 'checked';}?>>
				</td>
		...
		<?if($fieldType=='image'){?>
			<tr>
				...
				<td class="adm-detail-content-cell-r">
					<?
					list($fName, $val) = GetFieldEextraVal($SETTINGS, $fieldPrefix, 'PICTURE_WIDTH');
					?>
					<input type="text" name="<?=$fName?>" value="<?=htmlspecialcharsex($val)?>">
				</td>
			</tr>
			...
			<tr>
				...
				<td class="adm-detail-content-cell-r">
					<?
					list($fName, $val) = GetFieldEextraVal($SETTINGS, $fieldPrefix, 'PICTURE_HEIGHT');
					?>
					<input type="text" name="<?=$fName?>" value="<?=htmlspecialcharsex($val)?>">
				</td>
			</tr>

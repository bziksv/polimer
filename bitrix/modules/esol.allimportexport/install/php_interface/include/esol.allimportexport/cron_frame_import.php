...
\Bitrix\EsolAie\Import\Utils::RemoveTmpFiles(0); //Remove old dirs
...
foreach($arProfiles as $PROFILE_ID)
{
	...
	if($params['EXT_DATA_FILE'] || $params['EMAIL_DATA_FILE'])
	{
		$newFileId = 0;
		$fileLink = '';
		if(0 /*$params['EMAIL_DATA_FILE']*/)
		{
			/*if($newFileId = \Bitrix\KdaImportexcel\SMail::GetNewFile($params['EMAIL_DATA_FILE'], 86400, 'esil_allimport_'.$PROFILE_ID))
			{
		...
		{
			$arFile = \Bitrix\EsolAie\Import\Utils::MakeFileArray($params['EXT_DATA_FILE'], 86400);
		...
		else
		{
			if(!$newFileId && $arFile)
			{
				...
				$arFile['external_id'] = 'esil_allimport_'.$PROFILE_ID;
				$arFile['del_old'] = 'Y';
				$newFileId = \Bitrix\EsolAie\Import\Utils::SaveFile($arFile);
			}
		}
		...
		if($newFileId > 0)
		{
			...
			if($params['DATA_FILE']) \Bitrix\EsolAie\Import\Utils::DeleteFile($params['DATA_FILE']);
	...
	$arResult = $moduleRunnerClass::DoImport($arProfileFields['ENTITY'], $DATA_FILE_NAME, $params, $EXTRASETTINGS, $arParams, $pid);
	...
	echo date('Y-m-d H:i:s').": import complete\r\n"."Profile id = ".$PROFILE_ID."\r\n".\Bitrix\EsolAie\Utils::PhpToJSObject($arResult)."\r\n\r\n";
}
?>
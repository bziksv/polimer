...
function esol_allimportexport_demo_expired() {
	$DemoMode = CModule::IncludeModuleEx("esol.allimportexport");
	if ($DemoMode==MODULE_DEMO) {
		$now=time();
		if (defined("esol_allimportexport_OLDSITEEXPIREDATE")) {
			if ($now>=esol_allimportexport_OLDSITEEXPIREDATE || esol_allimportexport_OLDSITEEXPIREDATE>$now+2000000) {
				return true;
			}
		} else{ 
			return true;
...
function esol_allimportexport_show_demo($bAjax = false) {
	$moduleId = 'esol.allimportexport';
	$DemoMode = CModule::IncludeModuleEx($moduleId);
	$activateText = GetMessage("ESOL_AIE_DEMO_MESSAGE_ACTIVATE_MODULE",array("#LANG#"=>LANGUAGE_ID));
	if ($DemoMode==MODULE_DEMO) {
		$now=time();
		if (defined("esol_allimportexport_OLDSITEEXPIREDATE")) {
			if ($now<esol_allimportexport_OLDSITEEXPIREDATE) {
				print BeginNote();
				$expire_arr = getdate(esol_allimportexport_OLDSITEEXPIREDATE);
				...
				print GetMessage("ESOL_AIE_DEMO_MESSAGE_DAYS_REMAIN",array("#DAYS#"=>$days, "#ACTIVATE#"=>$activateText));
				print EndNote();
			} else {
				print BeginNote();
				print GetMessage("ESOL_AIE_DEMO_MESSAGE_EXPIRED", array("#ACTIVATE#"=>$activateText));
				print EndNote();
			}
		} else{ 
			print BeginNote();
	...
	else
	{
		...
		else
		{
			...
			if($updateEnd && is_callable(array('CUpdateClientPartner', 'GetLicenseKey')))
			{
				...
				echo '<div id="kda-ee-updates-message-inner">'.GetMessage("ESOL_AIE_DEMO_MESSAGE_UPDATE_END", array("#LCKEY#"=>$lckey, "#LANG#"=>LANGUAGE_ID)).'</div>';
			}
		}
	}

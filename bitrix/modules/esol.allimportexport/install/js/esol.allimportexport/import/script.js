...
var kdaIEModuleFilePrefix = 'esol_allimportexport_import';
...
var EList = {
			...
			select.bind('change', function(){
				if(this.value=='new_prop')
				{
					...
					var dialog = new BX.CAdminDialog({
						...
						'content_url':'/bitrix/admin/'+kdaIEModuleFilePrefix+'_new_property.php?FIELD_NAME='+encodeURIComponent(/*input.name*/input.id)+'&PROP_NAME='+encodeURIComponent(propName),
						'width':'600',
						'height':'400',
						'resizable':true});
	...
	CreateNewProperties: function(listIndex)
	{
		...
		if(items.length > 0)
		{
			var dialogParams = {
				...
				'content_url':'/bitrix/admin/'+kdaIEModuleFilePrefix+'_new_properties.php?list_index='+listIndex,
	...
	ShowLineActions: function(input, firstInit)
	{
		...
		var title = '';
		if(action.indexOf('SECTION_NAME_CELL')==0)
		{
		...
		else if(action.indexOf('SET_TITLES')==0)
		{
	...
	RemoveLineActions: function(input)
	{
		...
		if(action.indexOf('SET_TITLES')==0)
		{
			var levelType = (action.indexOf('SET_TITLES')==0 ? 'headers' : 'hints');
	...
	ShowFieldSettings: function(btn, name, val)
	{
		...
		var dialogParams = {
			...
			'content_url':'/bitrix/admin/'+kdaIEModuleFilePrefix+'_field_settings.php?field='+val+'&field_name='+name+'&PROFILE_ID='+form.PROFILE_ID.value+'&entity='+form.entity.value+'&count_cols='+countCols,
			'width':'900',
			'height':'400',
			'resizable':true
		};
...
var EProfile = {
	...
	ShowCron: function()
	{
		var cronWindowPath = '/bitrix/admin/'+kdaIEModuleFilePrefixShare+'_cron_settings.php'+window.location.search+'&type=import';
		var dialog = new BX.CAdminDialog({
...
$(document).ready(function(){
	if($('#esol_aie_preview_file').length > 0)
	{
		var post = $('#esol_aie_preview_file').closest('form').serialize() + '&ACTION=SHOW_REVIEW_LIST';
		$.post(window.location.href, post, function(data){
			$('#esol_aie_preview_file').html(data);

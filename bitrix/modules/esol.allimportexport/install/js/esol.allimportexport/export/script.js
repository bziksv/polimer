...
var kdaIEModuleFilePrefixShare = 'esol_allimportexport';
...
var EList = {
	Init: function()
	{
		...
		$(window).bind('resize', function(){
			EList.SetWidthList();
			$('.kda-ee-tbl-scroll').removeClass('fixed');
			$('.kda-ee-tbl-scroll-static').remove();
		});
		...
		$(window).bind('scroll', function(){
			...
			$('.kda-ee-tbl-wrap:visible').each(function(){
				var height = $(this).height();
				if(height <= windowHeight) return;
				var top = $(this).offset().top;
				var scrollDiv = $(this).closest('.kda-ee-sheet').find('.kda-ee-tbl-scroll');
				if(scrollTop > top && scrollTop < top + height - windowHeight)
				{
					if(!scrollDiv.hasClass('fixed'))
					{
						scrollDiv.before('<div class="kda-ee-tbl-scroll-static" style="height: '+(scrollDiv.height())+'px"></div>');
						scrollDiv.addClass('fixed');
					}
				}
				else if(scrollDiv.hasClass('fixed'))
				{
					$(this).closest('.kda-ee-sheet').find('.kda-ee-tbl-scroll-static').remove();
	...
	ShowFieldSettings: function(btn)
	{
		...
		var dialogParams = {
			...
			'content_url':'/bitrix/admin/'+kdaIEModuleFilePrefix+'_field_settings.php?field='+val+'&field_name='+name+'&PROFILE_ID='+form.PROFILE_ID.value+'&entity='+form.ENTITY.value,
			'width':'900',
			'height':'400',
			'resizable':true
		};
	...
	ShowFieldsListSettings: function(listIndex)
	{
		...
		var dialogParams = {
			...
			'content_url':'/bitrix/admin/'+kdaIEModuleFilePrefix+'_fields_list_settings.php?list_index='+listIndex+'&PROFILE_ID='+form.PROFILE_ID.value+'&entity='+form.ENTITY.value,
			'content_post': params,
			'width':'900',
			'height':'400',
			'resizable':true
	...
	SetLineDisplaySetting: function(key, listIndex, type)
	{
		...
		var dialogParams = {
			...
			'content_url':'/bitrix/admin/'+kdaIEModuleFilePrefix+'_display_settings.php?key='+key+'&list_index='+listIndex+'&type='+type+'&PROFILE_ID='+form.PROFILE_ID.value+'&entity='+form.ENTITY.value,
	...
	AddNewList: function(btn, event)
	{
		...
		if(copySettings)
		{
			...
			$('select[name^="SETTINGS[EFILTER]["]', parent).each(function(){
...
var EProfile = {
	...
	ShowCron: function()
	{
		var cronWindowPath = '/bitrix/admin/'+kdaIEModuleFilePrefixShare+'_cron_settings.php'+window.location.search+'&type=export';
		var dialog = new BX.CAdminDialog({
		...
		BX.addCustomEvent(dialog, 'onWindowRegister', function(){
			...
			{
				$('select.esol-chosen-multi').chosen({search_contains: true, placeholder_text: BX.message("ESOL_AI_CRON_CHOOSE_PROFILE")});
			}
...
function KdaEEFilterField(filterBlock, fieldPrefix, fieldIndex, filterData, filterKey)
{
	...
	this.SetCondEqList = this.SetCondNeqList = function(single, callback)
	{
		...
		{
			var obj = this;
			$.post(window.location.href, 'MODE=AJAX&ACTION=GET_FILTER_FIELD_VALS&FIELD='+this.fieldCode+/*'&ETYPE='+$('#ETYPE').val()+*/'&ENTITY_ID='+$('input[name="ENTITY_ID"]', this.filterBlock.closest('.kda-ee-sheet-cfilter')).val(), function(data){
				var newInput = $('<input type="hidden" name="'+valsInputName+'" value="">');
				newInput.val(data);
				obj.filterBlock.find('input[name="ENTITY_ID"]').after(newInput);

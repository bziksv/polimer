...
if (!\Bitrix\Main\Loader::includeModule('yandex.market') || !Access::isReadAllowed()) {
...
// Смотреть настройки можно по праву чтения, менять опции и секреты интеграции — только по праву записи.
$canSave = Access::isWriteAllowed();

// Секреты интеграции: оператору без права записи вместо значения показываем маску.
$secretOptions = array('JWT_TOKEN', 'YANDEX_KIT_CREDENTIALS', 'PRODUCTS_PRICES_OAUTH_TOKEN');
...
$arProductsAvailabilityStores = array();
if (\Bitrix\Main\Loader::includeModule('catalog') && class_exists('\Bitrix\Catalog\StoreTable')) {
    $storeResult = \Bitrix\Catalog\StoreTable::getList([
        'filter' => [ '=ACTIVE' => 'Y' ],
    ...
    while ($store = $storeResult->fetch()) {
        ...
        if ($title === '') {
            $title = trim((string)$store['ADDRESS']);
        }
        if ($title === '') {
            $title = 'Склад #' . $store['ID'];
        }
        $arProductsAvailabilityStores[(string)$store['ID']] = '[' . $store['ID'] . '] ' . $title;
    }
}
...
$productsAvailabilityPendingCount = 0;
...
if (\Bitrix\Main\Loader::includeModule('yandex.market') && class_exists('\Yandex\Market\Products\Availability\Repository')) {
    try {
        $productsAvailabilityPendingCount = (int)\Yandex\Market\Products\Availability\Repository::countPending();
    } catch (\Throwable $e) {
        $productsAvailabilityPendingCount = 0;
    }
}
...
$aTabs = array(
    ...
    array(
        ...
        'OPTIONS' => array(
            ...
            array('__group_basket_button_look', 'Внешний вид кнопки'),
            array('YAKIT_BUTTON_CAPTION', 'Текст кнопки', 'checkout', array('select', array(
                'checkout' => 'Оформить заказ',
                'continue' => 'Продолжить покупку',
            ))),
            array('YAKIT_BUTTON_THEME', 'Тема кнопки', 'gradient', array('select', array(
                'gradient' => 'Градиент',
                'black' => 'Чёрная',
                'white' => 'Белая',
                'whiteOutlined' => 'Белая с обводкой',
                'custom' => 'Свой цвет',
            ))),
            array('YAKIT_BUTTON_COLOR', 'Свой цвет фона (HEX, напр. #1a1a1a)', '', array('text', 20)),
            array('YAKIT_BUTTON_WIDTH', 'Ширина: max, auto или число px', 'max', array('text', 20)),
            array('YAKIT_BUTTON_HEIGHT', 'Высота кнопки, px (40–60)', '44', array('text', 20)),
            array('YAKIT_BUTTON_RADIUS', 'Скругление углов, px', '12', array('text', 20)),
            array('YAKIT_BUTTON_WRAP_CLASS', 'CSS-класс контейнера кнопки (необязательно)', '', array('text', 60)),
    ...
    array(
        'DIV' => 'edit7',
        'TAB' => 'Отслеживание цен и наличия',
        'OPTIONS' => array(
            array('__group_products_prices', 'Partner API: цены и наличие'),
            array('PRODUCTS_PRICES_ENABLED', 'Включить отслеживание цен и наличия', 'N', array('checkbox')),
            ...
            array('__group_products_availability', 'Источник остатков для наличия'),
            array('PRODUCTS_AVAILABILITY_STOCK_MODE', 'Режим учёта остатков', 'general', array('select', array(
                'general' => 'Общий остаток товара',
                'warehouse' => 'Остатки на складах',
            ))),
            array('PRODUCTS_AVAILABILITY_STORE_IDS', 'Склады для учёта остатков', '', array('multiselect', $arProductsAvailabilityStores)),
...
if ($canSave && $request->isPost() && check_bitrix_sessid() && $request->getPost('generate_token')) {
...
if ($request->isPost() && check_bitrix_sessid() && $request->getPost('test_connection')) {
    ...
    // Значение из формы принимаем только от оператора с правом записи: без него в поле
    // стоит маска, и диагностика должна работать по сохранённому в опциях значению.
    $credentials = ($canSave ? $request->getPost('YANDEX_KIT_CREDENTIALS') : '')
        ?: Option::get($module_id, 'YANDEX_KIT_CREDENTIALS', '');
...
if ($request->isPost() && check_bitrix_sessid() && $request->getPost('load_products_prices_feeds')) {
    ...
    $token = trim((string)(($canSave ? $request->getPost('PRODUCTS_PRICES_OAUTH_TOKEN') : '') ?: Option::get($module_id, 'PRODUCTS_PRICES_OAUTH_TOKEN', '')));
...
if ($canSave && $request->isPost() && check_bitrix_sessid() && !$tokenGenerated) {
    foreach ($aTabs as $aTab) {
        foreach ($aTab['OPTIONS'] as $arOption) {
            ...
            if ($optionName == 'YANDEX_KIT_CREDENTIALS') {
                ...
                if ($optionName == 'YAKIT_BUTTON_THEME') {
                    $value = in_array((string) $value, ['gradient', 'black', 'white', 'whiteOutlined', 'custom'], true) ? (string) $value : 'gradient';
                }
                if ($optionName == 'YAKIT_BUTTON_CAPTION') {
                    $value = ((string) $value === 'continue') ? 'continue' : 'checkout';
                }
                if ($optionName == 'YAKIT_BUTTON_COLOR') {
                    $color = trim((string) $value);
                    $value = preg_match('/^#([0-9a-fA-F]{3}|[0-9a-fA-F]{6})$/', $color) ? $color : '';
                }
                if ($optionName == 'YAKIT_BUTTON_WIDTH') {
                    $width = trim((string) $value);
                    if ($width !== 'max' && $width !== 'auto') {
                        $width = (int) $width;
                        $value = $width > 0 ? (string) $width : 'max';
                    } else {
                        $value = $width;
                    }
                }
                if ($optionName == 'YAKIT_BUTTON_HEIGHT') {
                    $value = (string) min(60, max(40, (int) $value));
                }
                if ($optionName == 'YAKIT_BUTTON_RADIUS') {
                ...
                if ($optionName === 'PRODUCTS_AVAILABILITY_STOCK_MODE') {
                    $value = $value === 'warehouse' ? 'warehouse' : 'general';
                }
    ...
    // Тема "Свой цвет" без валидного HEX бессмысленна (SDK нарисует градиент):
    // не храним сломанную комбинацию — откатываем тему на градиент.
    if (Option::get($module_id, 'YAKIT_BUTTON_THEME', 'gradient') === 'custom'
        && trim((string) Option::get($module_id, 'YAKIT_BUTTON_COLOR', '')) === '') {
        Option::set($module_id, 'YAKIT_BUTTON_THEME', 'gradient');
    }
    ...
    if (\Bitrix\Main\Loader::includeModule('yandex.market') && class_exists('\Yandex\Market\Products\Availability\Installer')) {
        try {
            \Yandex\Market\Products\Availability\Installer::install();
        } catch (\Throwable $e) {
        }
    }
}
    ...
    <? foreach ($aTabs as $aTab): ?>
        ...
        <? foreach ($aTab['OPTIONS'] as $arOption):
                ...
                <td colspan="2" style="padding-bottom: 12px;">
                    <div class="ycp-products-prices-help">
                        <p style="margin: 0 0 8px 0;">
                            Модуль отслеживает изменения цен и остатков в каталоге и отправляет их в
                            <a href="https://yandex.ru/dev/products/doc/ru/reference/post-offer-prices-updates" target="_blank" rel="noopener">Partner API поиска по товарам</a>
                            (Яндекс Товары): цены через <code>offer-prices/updates</code>, при нулевом остатке — скрытие через
                            <a href="https://yandex.ru/dev/products/doc/ru/reference/post-hidden-offers" target="_blank" rel="noopener">hidden-offers</a>,
                            при появлении остатка — возобновление показа через
                            <a href="https://yandex.ru/dev/products/doc/ru/reference/delete-hidden-offers" target="_blank" rel="noopener">DELETE hidden-offers</a>.
                            ID товара в запросе берётся из настройки «Идентификатор товара» на вкладке «Основные настройки».
                        </p>
            ...
            <tr>
                ...
                <?php endif; ?>
                <?php if ($aTab['DIV'] == 'edit7' && $arOption[0] === '__group_products_availability') : ?>
            <tr>
                <td colspan="2" style="padding-bottom: 8px;">
                    <div class="ycp-products-prices-help">
                        <p style="margin: 0;">
                            При скрытии и показе офферов модуль опирается на выбранный источник остатков.
                            В режиме «Остатки на складах» можно указать один или несколько складов.
                            Если склады не выбраны, учитываются все активные склады каталога.
                        </p>
                    </div>
                </td>
            </tr>
            ...
            <tr>
                <td colspan="2" style="padding-bottom: 8px;">
                    <div class="ycp-products-prices-help">
                        <p style="margin: 0;">
                            После сохранения модуль ставит изменения цен и остатков в очередь и отправляет их в Яндекс с указанным интервалом.
                            <? if ($productsPricesPendingCount > 0): ?>
                                В очереди цен: <strong><?= (int)$productsPricesPendingCount ?></strong>
                                <?= $productsPricesPendingCount === 1 ? 'товар' : 'товар(ов)' ?>.
                            <? endif; ?>
                            <? if ($productsAvailabilityPendingCount > 0): ?>
                                В очереди наличия: <strong><?= (int)$productsAvailabilityPendingCount ?></strong>
                                <?= $productsAvailabilityPendingCount === 1 ? 'товар' : 'товар(ов)' ?>.
                            <? endif; ?>
                        </p>
                    </div>
                </td>
            ...
            } elseif ($arOption[0] == 'PRODUCTS_AVAILABILITY_STORE_IDS') {
                $val = Option::get($module_id, 'PRODUCTS_AVAILABILITY_STORE_IDS', '');
                if (!empty($val)) {
                    $unserialized = \Yandex\Market\Utils\PhpSerializer::decode($val);
            ...
            <tr <? if ($arOption[0] == 'AUTO_CANCEL_STATUS'): ?>id='auto_cancel_status_row' style='display: <?= (Option::get($module_id, 'AUTO_CANCEL_ON_STATUS_CHANGE', 'N') == 'Y' ? '' : 'none') ?>;'<? endif; ?>
                ...
                <? if ($arOption[0] == 'YAKIT_BUTTON_COLOR'): ?>id='yakit_button_color_row' style='display: <?= (Option::get($module_id, 'YAKIT_BUTTON_THEME', 'gradient') === 'custom' ? '' : 'none') ?>;'<? endif; ?>
                <? if ($arOption[0] == 'DEFAULT_PRODUCT_QUANTITY'): ?>id='default_quantity_row'<? endif; ?>
                <? if ($arOption[0] == 'PRODUCTS_AVAILABILITY_STORE_IDS'): ?>id='products_availability_store_ids_row' style='display: <?= (Option::get($module_id, 'PRODUCTS_AVAILABILITY_STOCK_MODE', 'general') === 'warehouse' ? '' : 'none') ?>;'<? endif; ?>>
                ...
                <td width='60%'>
                    ...
                    <? if ($arOption[3][0] == 'text'): ?>
                        <?php
                        $isSecretOption = in_array($arOption[0], $secretOptions, true);
                        $displayValue = ($isSecretOption && !$canSave && (string)$val !== '') ? '********' : $val;
                        ?>
                        <input type='text' name='<?= htmlspecialcharsbx($arOption[0]) ?>' value='<?= htmlspecialcharsbx($displayValue) ?>'<?= $canSave ? '' : ' readonly' ?>
                    ...
                    <? if ($arOption[3][0] == 'textarea'): ?>
                        <textarea name='<?= htmlspecialcharsbx($arOption[0]) ?>' id='<?= htmlspecialcharsbx($arOption[0]) ?>'<?= $canSave ? '' : ' readonly' ?>
                    ...
                    <? if ($arOption[3][0] == 'select'): ?>
                        <select name='<?= htmlspecialcharsbx($arOption[0]) ?>'
                            ...
                            <? endif; ?>
                            <? if ($arOption[0] == 'YAKIT_BUTTON_THEME'): ?>
                                id='yakit_button_theme_select'
                            <? endif; ?>
                            <? if ($arOption[0] == 'PRODUCTS_AVAILABILITY_STOCK_MODE'): ?>
                                id='products_availability_stock_mode' onchange='toggleProductsAvailabilityStores()'
                            <? endif; ?>
                            ...
                            <? if (!in_array($arOption[0], [
                                ...
                                'YA_PAY_WIDGET_THEME',
                                'PRODUCTS_AVAILABILITY_STOCK_MODE',
                            ])): ?>
                    ...
                    <? endif; ?>
                    <? if ($arOption[3][0] == 'multiselect' && $arOption[0] == 'PRODUCTS_AVAILABILITY_STORE_IDS'): ?>
                        <select name='<?= htmlspecialcharsbx($arOption[0]) ?>[]' id='products_availability_store_ids' multiple size='8' style='width: 360px;' class='yastore-main-option-input'>
                            <? foreach ($arOption[3][1] as $optValue => $optName): ?>
                                <option value='<?= htmlspecialcharsbx($optValue) ?>' <?= (is_array($val) && in_array((string)$optValue, array_map('strval', $val), true) ? 'selected' : '') ?>>
                                    <?= htmlspecialcharsbx($optName) ?>
                                </option>
                            <? endforeach; ?>
                        </select>
                        <? if (empty($arOption[3][1])): ?>
                            <div style='margin-top: 6px; color: #666;'>Активные склады в каталоге не найдены.</div>
                        <? else: ?>
                            <div style='margin-top: 6px; color: #666;'>Удерживайте Ctrl (Cmd на Mac), чтобы выбрать несколько складов.</div>
                        <? endif; ?>
                    <? elseif ($arOption[3][0] == 'multiselect'): ?>
            ...
            <? if ($arOption[0] == 'JWT_TOKEN' && $canSave): ?>
            <tr>
                <td width='40%'></td>
                <td width='60%'>
...
<script>
    function toggleProductsAvailabilityStores() {
        var modeSelect = document.getElementById('products_availability_stock_mode');
        var storesRow = document.getElementById('products_availability_store_ids_row');
        if (modeSelect && storesRow) {
            storesRow.style.display = modeSelect.value === 'warehouse' ? '' : 'none';
        }
    }
...
<script>
(function () {
    function applyButtonColorRow() {
        var sel = document.getElementById('yakit_button_theme_select');
        var row = document.getElementById('yakit_button_color_row');
        if (sel && row) {
            row.style.display = (sel.value === 'custom') ? '' : 'none';
        }
    }
    document.addEventListener('change', function (e) {
        if (e.target && e.target.id === 'yakit_button_theme_select') { 
            applyButtonColorRow();
        }
    });
    if (document.readyState === 'loading') {
        document.addEventListener('DOMContentLoaded', applyButtonColorRow);
    } else {
        applyButtonColorRow();
    }
})();

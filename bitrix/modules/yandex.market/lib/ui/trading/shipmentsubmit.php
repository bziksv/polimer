	...
	protected function makeItemInstances(ShipmentRequest\BasketItem $basketItem)
	{
		...
		foreach ($basketItem->getIdentifiers() as $index => $instances)
		{
			...
			foreach ($instances as $type => $value)
			{
                ...
                else if ($type === Market\Data\Trading\MarkingRegistry::COUNTRY_CODE)
                {
                    $key = Market\Data\Trading\CountryCode::formatMarkingCode($key);

                    if (!Market\Data\Trading\CountryCode::isValidCountryCode($value))
                    {
                        $value = null;
                    }
                }

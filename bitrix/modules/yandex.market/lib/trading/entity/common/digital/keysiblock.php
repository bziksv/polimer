...
class KeysIblock extends Skeleton
{
    ...
    protected const DG_POOL_PER_ITEM = 10;
	...
	protected function makeBasketItemCodes(TradingReference\Order $order, $basketCode, array $iblockElement, $quantity)
	{
		...
		Assert::notNull($iblockId, 'DIGITAL_SETTINGS[IBLOCK]');
        $fetchCount = max((int)$quantity * self::DG_POOL_PER_ITEM, (int)$quantity);
        ...
        foreach ($ids as $id) {
            if (count($result) >= (int)$quantity) { break; }

            $key = $candidates[$id];

            try {
                // Резервируем выбранный ключ
                $this->updateKey($id, [
                    'ACTIVE'                    => 'N',
                    $orderProperty              => $order->getId(),
                ...
                // если был гонка и ключ уже забрали — пробуем следующий
                continue;
            }
        }

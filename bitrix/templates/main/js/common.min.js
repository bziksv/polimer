$(document).ready(function() {
	...
	BX.addCustomEvent('onAjaxSuccess', function(res, res2){
		$('input[autocomplete="tel"]').mask("+7-999-999-99-99", { placeholder: "_", autoclear: false });
	});
	
    $(".phone").mask("+7-999-999-99-99", { placeholder: "_", autoclear: false });
    ...
    function scrollMobileBreadcrumbsToEnd() {
        $('.breadcrumbs-mob').each(function () {
            var el = this;
    ...
    scrollMobileBreadcrumbsToEnd();
    $(window).on('load', scrollMobileBreadcrumbsToEnd);
   ...
   */
    ...
    if ($('#imageGallery').length) {
        var $productGallery = $('#imageGallery');
        var $productGalleryWrap = $productGallery.closest('.pc__prod-gallery');
        var isMobileProductGallery = function () {
            return window.innerWidth <= 659;
        };
        var prepareProductGalleryLayout = function () {
            if (isMobileProductGallery()) {
                $productGalleryWrap.css({ width: '100%', maxWidth: '100%', marginLeft: 0, marginRight: 0, float: 'none' });
                $productGallery.css({ width: '', maxWidth: '', marginLeft: '', marginRight: '' });
            } else {
                $productGalleryWrap.css({ width: '', maxWidth: '', marginLeft: '', marginRight: '', float: '' });
                $productGallery.css({ width: '', maxWidth: '', marginLeft: '', marginRight: '' });
            }
            if ($productGallery[0]) {
                void $productGallery[0].offsetWidth;
            }
        };
        var refreshProductGallery = function () {
            prepareProductGalleryLayout();
            if (LSlider && typeof LSlider.refresh === 'function') {
                LSlider.refresh();
                if (typeof LSlider.goToSlide === 'function') {
                    LSlider.goToSlide(0);
                }
            }
        };
        var initProductGallery = function () {
            prepareProductGalleryLayout();
            LSlider = $productGallery.lightSlider({
                enableDrag:false,
                gallery:true,
                item:1,
                thumbItem:4,
                ...
                currentPagerPosition: isMobileProductGallery() ? 'middle' : 'left',
                mode:'slide',
            ...
            if (isMobileProductGallery()) {
                [50, 200, 500].forEach(function (delay) {
                    setTimeout(refreshProductGallery, delay);
                });
            }
        };
        requestAnimationFrame(function () {
            requestAnimationFrame(initProductGallery);
        });
        $(window).on('load resize orientationchange', refreshProductGallery);
        $productGallery.find('img').on('load', refreshProductGallery);
    }
    ...
    pc_tab.on('click', '.mtb', function(e) {
        e.preventDefault();
        $(this).closest('.tab').toggleClass('m_active');
    });
    ...
    if (window.matchMedia("(min-width: 660px)").matches){
        /* add2cart2 popup: handled in /js/function.js */
    }
    ...
    */
    ...
    var a = $('.ct__mask')
    var b = $('.products_roll .pr_header .filter')

    b.on('click', function(e) {
        e.preventDefault()
        /* ≤1319: one path via function.js helpers (portal + body class).
           Was 1019 before — at 1020–1319 common toggle + document handler double-toggled and closed immediately. */
        if (window.matchMedia('(max-width: 1319px)').matches && typeof polimerToggleMobileFilter === 'function') {
    ...
    a.on('click', function() {
        if (window.matchMedia('(max-width: 1319px)').matches && typeof polimerCloseMobileFilter === 'function') {
    ...
    if($('.compare-items').length){
        // На десктопе — kinetic; на мобилке только нативный overflow-x,
        // иначе kinetic глушит touchend/click (крестик удаления не работает на телефоне).
        if (window.matchMedia("(min-width: 1019px)").matches){
            $('.compare-items').kinetic({
                x: true,
                y: false,
                filterTarget: function(target){
                    return !$(target).closest('a, button, .delete, .add2basket').length;
                }
            });
        }
    }

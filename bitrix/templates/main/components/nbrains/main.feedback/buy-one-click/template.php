...
<div class="popup" id="oneclick"<?= $showForm ? ' style="display:block"' : '' ?>>
	<a href="#" class="close">&nbsp;</a>
			...
			?>
				...
				<?if($field['PROPERTY_TYPE'] == "S"):?>
						...
						<?else:?>
							...
							<span class="value">
								<select name="<?=$field['CODE']?>" required>
									...
									while ($arProp = $storeResult->GetNext()){
										$selected = (($arResult[$field['CODE']] ?? '') === $arProp['ADDRESS']) ? ' selected' : '';
										?>
										<option value="<?=$arProp['ADDRESS']?>"<?=$selected?>><?=$arProp['ADDRESS']?></option>
										<?
									}
									?>
								</select>
							</span>
							<?if($fieldInvalid):?><span class="polimer-field-error">Выберите точку самовывоза</span><?endif;?>
						</span>
						<? endif;?>
